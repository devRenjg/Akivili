...
async def run_probe(paths: dict, keep: bool) -> Probe:
    ...
    async def fake_dispatch(task_obj, agent_obj, prompt):
        slug = agent_obj["slug"]
        run_id_seq["n"] += 1
    ...
    # Shrink the timeout so the hang test is fast (production stays at 360s).
    original_timeout = collab.RUN_TIMEOUT_SEC
    collab.RUN_TIMEOUT_SEC = 1
    ...
    try:
        ...
        probe.check("卡死执行在超时附近返回（未无限阻塞）",
                    hang_elapsed < 30, f"elapsed={hang_elapsed:.2f}s (timeout={collab.RUN_TIMEOUT_SEC}s)")
        ...
        async def mixed_dispatch(task_obj, agent_obj, prompt):
            slug = agent_obj["slug"]
            run_id_seq["n"] += 1
        ...
        collab.RUN_TIMEOUT_SEC = original_timeout
