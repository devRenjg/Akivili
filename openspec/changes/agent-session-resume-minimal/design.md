...
> **⚠️ 实测修正（2026-07-24 真起 CLI 端到端联调）：「省历史重放」只在长对话成立，短对话 resume 反而更贵。**
> 真起 claude + codex 各跑「任务A 记暗号 → 任务B(跨 task resume) 问暗号」实测（见 `Papers/Session-Resume端到端实测-token与记忆.md`）：
> - **功能完全正确**：两线 session 捕获/跨 task 缓存/resume 续接/rollout 校验全部真实工作；**会话记忆连续**——两 CLI 在任务B 都答出了只在任务A 对话里出现的暗号（紫色犀牛-7391），证明会话真续上。
> - **token 反直觉**：短会话 resume **不省反增**——claude A=41041→B=41156（+115）、codex A=42452→B=91440（+48988，+115%）。
> - **根因**：本平台两模式的对比不是「全量历史 vs 增量历史」，而是「平台精简重放 `_clip_history`（双限裁剪）」vs「CLI 从磁盘 rollout 恢复**完整会话状态**（全部轮次/工具/系统上下文）」。resume 恢复的完整状态，在短会话里比平台裁剪后的 history 更大 → 更贵。
> - **修正结论**：resume 省 token 的前提是 **平台重放的 history ≥ CLI 恢复的会话状态**，即**长对话**（多轮累积、裁剪后仍很大）。短对话下 resume 的价值是**会话连续性/上下文保真**（记忆不丢），不是省 token。
> - **对 best-effort 取舍无影响**：resume 是「优化/连续性」而非正确性，miss 降级全量仍无害；只是「优化」的收益边界比原假设窄。阶段二保留（连续性价值真实存在），但不宣称短会话省 token。
> - **潜在后续（非本 change）**：若要让 token 收益兑现，可给 S5.3 加「仅当平台 history 规模超阈值才走跨 task resume」的门控——短会话不 resume（省下的重放本就少、还不如首建便宜）。属新设计决策，留 backlog。
