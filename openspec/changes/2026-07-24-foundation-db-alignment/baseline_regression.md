...
- **S5 修订（2026-07-24，全仓零 sqlite）**：主套件基线由 31/31 降为 **30/30**——「任务列表 p95<300ms」延迟断言改为只采集 metrics（不卡阀值），因它是机器速度依赖的微基准，在共享/冷启动的 CI runner 上偶发超阈红门禁（本地 ~85ms、CI 首跑超 300ms），属反模式。这是**主动降级 1 条断言**，非回归。同期退役 `run_wal_concurrency_probe`（WAL 随 sqlite 消亡）+ `run_orm_engine_probe`（PRAGMA/busy_timeout PG 无对应物），新建 `run_pg_concurrency_probe`；门禁探针数 40→39、全跑在 PostgreSQL 上（39/39·580 断言）。详见 `TestReport/README.md` 与 [[s5-postgres-only-progress]]。
