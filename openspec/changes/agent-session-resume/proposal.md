...
- **per-(task, agent) session + 串行折叠**：每个 `(conversation_id, agent_slug)` 维护一条 CLI session。新增轻表 `agent_sessions(conversation_id, agent_slug, session_id, committed_msg_id, provider_id, backend, workdir, updated_at)`,唯一键 `(conversation_id, agent_slug)`。同 (task, agent) **至多一个 active run（queued/claimed/running）**，重复触发**折叠**进下一轮（合并触发意图，**不丢**）——现状代码是 `return None` 丢弃（Review P0-3 修正）;DB 级唯一性用**存在的** `(task_id, agent_slug)` partial unique index `WHERE status IN ('queued','claimed','running')` 保证（应用层查重有 TOCTOU 竞态，不足以防并发 POST，Review P0-B），配合折叠模型（queued 原子合并水位 / claimed·running 记 pending intent、收尾据 pending 建至多一个 successor）。
