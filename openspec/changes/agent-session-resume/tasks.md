...
- [ ] 1.1 建表 `agent_sessions(id, conversation_id, agent_slug, session_id, snapshot_msg_id, provider_id, backend, workdir, updated_at)`,唯一键 `(conversation_id, agent_slug)`;`database.py` 建表 + 迁移（存量无行,首次执行自然走全量分支）
- [ ] 1.2 **同 (task, agent) 串行**：加约束/检查使同一 (conversation, agent) 至多一个 queued/running run;重复触发折叠进下一轮（复用现有自触发守卫思路 + partial unique index）
...
- [ ] 1.5 首次执行（无 session 行）走现状全量回灌,不带 resume,成功后落 session（session_id + provider_id + backend + workdir + snapshot_msg_id）
- [ ] 1.6 探针 `agent_session_build_probe`：首次执行后 `agent_sessions` 有正确 session_id、snapshot_msg_id = 构建 prompt 时的 MAX(messages.id)、provider/backend/workdir 落库;同 (task,agent) 不并行起两条

## S2 — claude resume + 增量回灌（快照水位，修正并发漏话）
- [ ] 2.1 `runner.py` 执行前查 `agent_sessions`,命中且 backend/provider/workdir 一致 → 走「增量 + resume」；否则走「全量首建」
- [ ] 2.2 **快照水位**：`snapshot_msg_id` = 构建本次 prompt 那刻的 `MAX(messages.id)`;水位只推进到快照,**不含执行期间别人新写的**（修正「完成时 MAX(id)」在并发下漏话）
- [ ] 2.3 增量历史 SQL：`messages WHERE conversation_id=? AND id > snapshot_msg_id AND 作者非本 agent ORDER BY id`（参数化 `id > ?`）;本 Agent 自产历史由 session 记忆承载不重喂;仍过 `_clip_history` 防单次海量
...
- [ ] 2.6 **resume 落地判定**：请求了 resume 但 stderr 出现 "no conversation found"、或输出 session_id≠请求 id → 判 resume 未落地 → 清 sid + 本次降级全量
...
- [ ] 4.1 `codex.py` 从一次性 `codex exec --json -m - -` 改为 **`codex app-server --listen stdio://` 长驻 + JSON-RPC**
