...
- [ ] 1.4 session_id 生命周期 = **session owner 四阶段 CAS 协议（Review P1-3 / 第四轮 P0-4）**：三件套（pin/覆盖/COALESCE 空值保护）之上落 acquire→pin→final→retire——**(1) acquire** 新 attempt 启动时按观察到的旧 `session_version` CAS 把 `current_task_run_id` 改为本 attempt、epoch+1（`RETURNING` 拿 owner_version;首次无行用 `INSERT ... ON CONFLICT DO NOTHING` 归属本 attempt）**(2) pin** 流中途只允许 `WHERE current_task_run_id=? AND session_version=owner_version` 更新 session_id，pin **不递增** version（epoch 只在 owner 切换/退休变）**(3) final** 成功收尾同事务更新 pointer+committed+backlog/pending successor **(4) retire** attempt 终态后清 `current_task_run_id`/推进 epoch，使迟到 pin/final CAS 失败;**终态无 successor 也 retire**;poisoned/降级清 session 与 retire 同事务。防新 attempt 首次 pin 不满足 WHERE（旧协议缺 acquire 的死锁）+ 防上一轮迟到事件覆盖新 pointer
...
- [ ] 2.3d **backlog 副作用防重 = prompt 约束轻方案（Review 第四轮 P0-3，用户拍板）**：**不禁用工具调用**（Claude/Codex 无「只注入上下文不产生 turn」通用接口）;`trigger=history_backlog` 的 prompt 显式声明「历史上下文同步、非新指令、勿重复执行已完成操作」+ 不计 mention-chain + 不触发任务自动流转;探针 `history_backlog_side_effect_observe_probe`（三批 backlog 中植入可观测副作用工具，**观测**是否重复调用/写普通 assistant message/触发 mention-auto-flow，记录残余风险数据供实现期评估是否升级 context-only 重方案;轻方案下不作硬断言，仅采集）;残余风险确认严重时再回头评估重方案
...
- [ ] 2.7c **session owner CAS 探针（Review 第四轮 P0-4 故障注入 3/4）**：`session_owner_acquire_probe`(attempt#1 完成后 attempt#2 原子取得 owner，attempt#1 迟到 pin/final 全 CAS 失败、attempt#2 pointer 不被覆盖)、`session_owner_retire_probe`(attempt 终态且无 successor 时注入迟到事件，因 retire 已清 owner，迟到写被拒、不改 session)
