...
5. running NULL → 先 generation/attempt fencing → kill/containment，按进程树退出确认状态分两支（第十五轮 P1-3，`orphaned` 全局语义 = 进程未确认退出，SHALL NOT 让已确认退出的旧进程永久显示「孤儿·未确认退出」污染 Runtime/失败归因/人工按钮）：
   5a. **未确认完整进程树退出** → attempt 落 `orphaned`（未确认死亡口径）+ cleanup blocker = unresolved + execution 落 `recovery_blocked(process_not_confirmed_dead)`;activate SHALL fail-closed（③ 存活进程树非零）。
   5b. **已确认完整进程树退出**（无残留进程） → attempt 落**已定义终态 `abandoned`**（用户第十五轮拍板复用 abandoned = 无残留进程，SHALL NOT 发明新状态词;`abandoned` 语义扩为「无残留进程」——含 claimed·CLI 从未起 与 running·已确认退出两种，见 PGR 词汇表）+ cleanup blocker = resolved + execution 落**已定义终态 `recovery_blocked(blocked_reason=null_conversation_migration)`**（复用 recovery_blocked，SHALL NOT 发明「人工隔离态」新 status;保留 pid/create_time/generation/instance/containment 待人工迁移）。
   两支下 execution 均保持 `recovery_blocked` 终态、SHALL NOT 假装迁移完成、SHALL NOT 在此建 child;是否可回队由 execution status（recovery_blocked=不回队）决定，**与 attempt 是否 `abandoned` 解耦**——`abandoned` 只表「无残留进程」，不隐含「回队」（回队是 execution 处置属性，NULL task 三层硬门本就不回队）。
6. 校验 activate readiness = 三段全零（第十四轮 P1-2，仅 active NULL count 不足以表达 fail-closed）：
   ...
   ├ unresolved NULL migration blocker count（execution recovery_blocked 且 blocked_reason=null_conversation_migration 且未确认清理）= 0
...
| running NULL·已确认退出 | `abandoned`（已确认进程树退出，第十五轮 P1-3） | `recovery_blocked(null_conversation_migration)` | 指向该 `abandoned` attempt | 保留 |
| running NULL·未确认退出 | `orphaned` | `recovery_blocked(process_not_confirmed_dead)`（**非 null_conversation_migration**——先解决进程再迁移） | 指向该 `orphaned` attempt | 保留 |
...
#### Scenario: running NULL 未确认退出 → orphaned + activate fail-closed（第十五轮 P1-3）
- **WHEN** activate 前存在 `conversation_id IS NULL` 的 `running` 行，其 CLI 仍在执行（持续写 sentinel 文件）、进程树未确认退出
- **THEN** SHALL 先 generation/attempt fencing → kill/containment，进程树**未确认**退出时 attempt 落 `orphaned`（未确认死亡）+ cleanup blocker unresolved + execution `recovery_blocked(process_not_confirmed_dead)`;activate SHALL fail-closed（③ 存活进程树非零，不假装迁移完成、不放任幽灵运行）

#### Scenario: running NULL 已确认退出 → abandoned + 等迁移（第十五轮 P1-3）
- **WHEN** running NULL 行经 fencing + kill/containment 后**已确认**完整进程树退出、无残留进程
- **THEN** attempt 落 `abandoned`（无残留进程，用户拍板复用、非 orphaned——SHALL NOT 让已确认退出的行显示「未确认退出」孤儿语义）+ cleanup blocker resolved + execution `recovery_blocked(null_conversation_migration)` 等人工迁移;`abandoned` 不隐含回队（NULL task 三层硬门不回队），回队与否由 execution status 决定
