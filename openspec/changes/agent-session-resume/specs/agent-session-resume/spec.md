...
5. running NULL → 先 generation/attempt fencing → kill/containment。**execution 从一开始就落 `recovery_blocked(blocked_reason=null_conversation_migration)`，进程清理状态用正交字段 `process_cleanup_state(unconfirmed|confirmed)` 表达（第十六轮 P1-C，SHALL NOT 用 `process_not_confirmed_dead` 作 blocked_reason——否则通用恢复会经 `resume_blocked_execution()` 建 `superseded_from=NULL 父` 的 recovery child，该 child conversation 仍 NULL、被三层硬门拒绝永远不可执行，迁移身份丢失）**。按 `process_cleanup_state` 分两支（`orphaned` 全局语义 = 进程未确认退出，SHALL NOT 让已确认退出的旧进程永久显示未确认孤儿）：
   5a. **未确认完整进程树退出**（`process_cleanup_state=unconfirmed`） → attempt 落 `orphaned` + execution `recovery_blocked(null_conversation_migration)` + `process_cleanup_state=unconfirmed`;activate SHALL fail-closed（③ 存活进程树非零）;**出口只允许「继续清理进程」、SHALL NOT 建任何 successor**。
   5b. **已确认完整进程树退出**（`process_cleanup_state=confirmed`，无残留进程） → attempt 落 `abandoned`（`abandon_stage=running`、`abandon_reason=null_conversation_migration`，第十六轮 P1-A）+ execution `recovery_blocked(null_conversation_migration)` + `process_cleanup_state=confirmed`（保留 pid/create_time/generation/instance/containment 待人工迁移）;**出口进入 null migration 事务、创建新 conversation 的普通 successor**（见「NULL 迁移出口」）。
   两支下 execution 均保持 `recovery_blocked(null_conversation_migration)` 终态不变、SHALL NOT 假装迁移完成、SHALL NOT 走 generic `resume_blocked_execution()` 建 `superseded_from` NULL recovery child;是否可回队由 execution status 决定（recovery_blocked=不回队），**与 attempt 是否 `abandoned` 解耦**——`abandoned` 只表「无残留进程」，不隐含回队。
...
| claimed NULL | `abandoned`（CLI 未起、无残留进程） | `recovery_blocked(null_conversation_migration)` | 指向该 `abandoned` attempt | 保留 |
| running NULL·已确认退出 | `abandoned`（`abandon_reason=null_conversation_migration`） | `recovery_blocked(null_conversation_migration)` + `process_cleanup_state=confirmed` | 指向该 `abandoned` attempt | 保留 |
| running NULL·未确认退出 | `orphaned` | `recovery_blocked(null_conversation_migration)` + `process_cleanup_state=unconfirmed`（**第十六轮 P1-C：blocked_reason 从头即 null_conversation_migration、清理状态用正交 process_cleanup_state，保住迁移身份、不走 generic recovery**） | 指向该 `orphaned` attempt | 保留 |
...
#### Scenario: running NULL 未确认退出 → orphaned + 只清理不建 successor（第十五轮 P1-3 + 第十六轮 P1-C）
- **WHEN** activate 前存在 `conversation_id IS NULL` 的 `running` 行，其 CLI 仍在执行（持续写 sentinel 文件）、进程树未确认退出
- **THEN** SHALL 先 generation/attempt fencing → kill/containment，进程树**未确认**退出时 attempt 落 `orphaned` + execution `recovery_blocked(null_conversation_migration)` + `process_cleanup_state=unconfirmed`（**blocked_reason 从一开始即 null_conversation_migration、非 process_not_confirmed_dead**，第十六轮 P1-C 保住迁移身份）;activate SHALL fail-closed;**出口只允许继续清理进程、SHALL NOT 走 generic resume 建 `superseded_from` NULL recovery child**（会永远不可执行）

#### Scenario: running NULL 已确认退出 → abandoned + 路由 migration（第十五轮 P1-3 + 第十六轮 P1-C）
- **WHEN** running NULL 行经 fencing + kill/containment 后**已确认**完整进程树退出、无残留进程
- **THEN** attempt 落 `abandoned`（`abandon_reason=null_conversation_migration`，非 orphaned——SHALL NOT 让已确认退出的行显示未确认孤儿）+ execution `recovery_blocked(null_conversation_migration)` + `process_cleanup_state=confirmed`;**出口进入 null migration 事务、创建新 conversation 普通 successor**（SHALL NOT 走 generic `resume_blocked_execution()` 建 NULL recovery child）;`abandoned` 不隐含回队，回队与否由 execution status 决定
...
- **THEN** SHALL 同时校验三段——① active NULL count（queued/claimed/running 且 conversation_id IS NULL）= 0;② unresolved NULL migration blocker count（execution `recovery_blocked(null_conversation_migration)` 且进程树未确认清理）= 0;③ surviving NULL process-tree count（残留 NULL 旧 CLI 进程树未确认退出）= 0;任一非 0 即 activate/readiness fail-closed，仅三段全零才放行
- **AND** running NULL 已落 `recovery_blocked` 使 active count 归零、但进程树未确认退出时，②/③ 非零仍 fail-closed（SHALL NOT 只凭 active count=0 误判可 activate）
...
- **THEN** SHALL 用 `null_migration_requests` 表 `{parent_execution_id, request_token, payload_hash, child_execution_id, target_conversation_id, actor, created_at}` + `UNIQUE(parent_execution_id, request_token)` 保证：同 token 重试返回同一 migration successor;不同 token 并发经 `UNIQUE(migration_from_execution_id)` 冲突后读回已存在 successor（`idempotent_replay=true`）、SHALL NOT 建第二个;同 token 不同 payload 返回 409;**active partial unique index 只挡同一时刻并发、SHALL NOT 依赖它防「首个 successor 完成后再次重放建第二个」**——持久因果键 `UNIQUE(migration_from_execution_id)` 才是防重放的真相源

### Requirement: NULL 隔离/迁移的独立终态事务、lineage 完整性与输家映射（第十六轮 P1-D）

**1. 独立 `quarantine_null_execution()` 终态事务**：queued NULL 无 attempt，SHALL NOT 机械复用「以定局 attempt 为中心」的 `finish_execution()`。三态隔离 SHALL 走独立事务 `quarantine_null_execution()`，在**同一提交**内完成：① source status CAS（queued/claimed/running → 隔离）;② attempt 状态（claimed→abandoned、running confirmed→abandoned、running unconfirmed→orphaned;queued 无 attempt 跳过）;③ owner retire + current/final pointer（claimed/running 指定局 attempt、queued final=NULL）;④ pending intent 保留;⑤ execution 落 `recovery_blocked(null_conversation_migration)` + `process_cleanup_state`;⑥ `terminal(recovery_blocked)` event（遵全局 id 顺序）;⑦ migration audit。任一 CAS/唯一约束失败整体回滚。**SHALL NOT 让 execution 已终态但 SSE 无 terminal、或 owner/pointer/status 分开提交**（半提交防护同 `finish_execution()`）。
...
**3. `null_migration_requests` 不同 token 输家持久映射**（镜像 recovery request winner/loser，第十六轮 P1-D）：赢家在迁移事务内写 `{本 token → 新 successor}`;不同 token 输家 SHALL 在**失败事务回滚后的独立事务**里只写 `{本 token, payload_hash → existing successor}` 映射、不重复迁移副作用——否则输家响应再丢无法按 token 稳定重放、同 token 不同 payload 无法稳定 409。

#### Scenario: 不同 token 输家持久映射 existing successor
- **WHEN** 不同 token 并发迁移同一 NULL 父，输家因 `UNIQUE(migration_from_execution_id)` 冲突
- **THEN** 输家先回滚失败的 successor-insert 事务，再独立事务只写 `null_migration_requests(本 token, payload_hash → existing successor)` 映射、返回 `idempotent_replay=true`;SHALL NOT 重复建 successor/重复迁移消息/重复副作用;此后该 token 重试稳定返回同一 successor、同 token 不同 payload 稳定 409
