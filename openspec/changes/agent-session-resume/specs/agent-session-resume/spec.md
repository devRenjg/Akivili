...
- **THEN** SHALL 用 `null_migration_requests` 表 `{parent_execution_id, request_token, payload_hash, child_execution_id, target_conversation_id, actor, created_at, canonical_request_id, outcome(winner|idempotent_replay|rejected_already_resolved)}` + `UNIQUE(parent_execution_id, request_token)` 保证：同 token 重试返回同一 migration successor;**不同 token 并发经 `UNIQUE(migration_from_execution_id)` 冲突后 SHALL 比对 `payload_hash` 与赢家 `canonical_request_id`——payload 一致读回已存在 successor（`idempotent_replay=true`）、SHALL NOT 建第二个;payload 不同（不同目标 conversation/不同切分意图）返回 409 `already_resolved`（附赢家 successor，第十七轮 P1-D，与 PGR recovery request 同口径）、SHALL NOT 静默映射不同意图到赢家 successor**;同 token 不同 payload 返回 409;**active partial unique index 只挡同一时刻并发、SHALL NOT 依赖它防「首个 successor 完成后再次重放建第二个」**——持久因果键 `UNIQUE(migration_from_execution_id)` 才是防重放的真相源
...
- **鉴权**：SHALL 要求操作者身份（`actor`）记入审计（同 recovery resume）。
- **请求体**：`{migration_token, target_conversation(新建或指定的目标 conversation), message_split(可审计的消息切分规格), reason?}`——`migration_token` 为请求幂等键;切分规格 SHALL 可审计、无法可靠判定时服务端拒绝并保持 quarantine（不猜测迁移）。
...
- **幂等/冲突**：同 `migration_token` 重试返回同一 successor + `idempotent_replay=true`;不同 token·payload 一致返回已存在 successor + `idempotent_replay=true`;**不同 token·payload 不同**（不同目标 conversation/切分意图）返回 **409 `already_resolved`**（附赢家 successor/canonical_request_id）;父非 `recovery_blocked(null_conversation_migration)` 返回 409;`process_cleanup_state=unconfirmed` 返回 409「待清理」。
- **事务**：走 [platform-graceful-restart] 的迁移事务（建新 conversation + 消息切分 + 更新 `tasks.conversation_id` + 转移 pending intent + 建普通 successor + 写 `null_migration_requests`/lineage/audit + 写 resolved cache `resolution_kind='migration'`），全提交或全回滚。
...
**3. `null_migration_requests` 不同 token 输家按 payload 分两种结局**（镜像 recovery request winner/loser，第十六轮 P1-D + 第十七轮 P1-D）：赢家在迁移事务内写 `{本 token, canonical_request_id, outcome=winner → 新 successor}`;不同 token 输家 SHALL 在**失败事务回滚后的独立事务**里先比对 `payload_hash` 与赢家 `canonical_request_id`——**payload 一致**只写 `{本 token, payload_hash, outcome=idempotent_replay → existing successor}` 映射、返回 `idempotent_replay=true`;**payload 不同**（不同目标 conversation/不同切分意图）只写 `{本 token, payload_hash, outcome=rejected_already_resolved}` 审计、返回 409 `already_resolved`（附赢家 successor）——均不重复迁移副作用，否则输家响应再丢无法按 token 稳定重放、不同意图会被静默吞掉。
...
- **THEN** 输家先回滚失败的 successor-insert 事务，再独立事务比对 `payload_hash`==赢家 `canonical_request_id` 通过、只写 `null_migration_requests(本 token, payload_hash, outcome=idempotent_replay → existing successor)` 映射、返回 `idempotent_replay=true`;SHALL NOT 重复建 successor/重复迁移消息/重复副作用;此后该 token 重试稳定返回同一 successor
...
- **THEN** 输家 `UNIQUE(migration_from_execution_id)` 冲突后比对 `payload_hash` != 赢家 `canonical_request_id`，独立事务只写 `null_migration_requests(本 token, payload_hash, outcome=rejected_already_resolved)` 审计、返回 409 `already_resolved`（附赢家 successor）;SHALL NOT 把输家的不同迁移意图静默映射到赢家 successor、SHALL NOT 建第二个 successor
