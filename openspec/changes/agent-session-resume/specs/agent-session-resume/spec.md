...
- **请求体**：`{migration_token, target_conversation(新建或指定的目标 conversation), message_split(可审计的消息切分规格), expected_task_row_version, source_message_watermark, expected_cleanup_version, reason?}`——`migration_token` 为请求幂等键;切分规格 SHALL 可审计、无法可靠判定时服务端拒绝并保持 quarantine（不猜测迁移）。**🔴 快照 CAS 字段 SHALL 映射可执行 schema（第十九轮 P1-5 + 第二十轮 P1-5：现网 `tasks` 只有 `updated_at` 无单调 version）**：
  - **`expected_task_row_version`**：SHALL 新增 `tasks.row_version INTEGER NOT NULL DEFAULT 0` 单调列（每次相关修改递增），预览时取值、POST 时 CAS 比对（SHALL NOT 依赖 `updated_at` 字符串做并发判据）。
...
#### Scenario: 预览快照漂移后 /migrate 拒绝过期切分（第十九轮 P1-5）
- **WHEN** 操作者预览 message_split 后、POST /migrate 之前，源 task 被修改（`tasks.row_version` 递增）、源 conversation 新增消息（`source_message_watermark`=`MAX(messages.id)` 变化）、或 `process_cleanup_state` 经 `confirm_null_process_cleanup()` 推进（`cleanup_version` 变化）
- **THEN** 服务端在迁移事务内 CAS 比对 `expected_task_row_version`（对 `tasks.row_version`）/`source_message_watermark`（对当前 `MAX(messages.id)`）/`expected_cleanup_version`（running 来源精确匹配、queued/claimed 来源固定 0）与当前值不符，返回 409 `snapshot_drift` 要求重新预览;SHALL NOT 按过期切分提交（否则新增消息被漏迁或错切）;重新预览取最新快照后再 POST 才放行
