...
5. running NULL → 先 generation/attempt fencing → kill/containment 确认完整进程树退出
                 → attempt 落**已定义终态 `orphaned`**（running 未确认死亡口径，复用第九轮 orphaned，SHALL NOT 发明「迁移专用终态」新状态词）
                 → execution 落**已定义终态 `recovery_blocked`，`blocked_reason=null_conversation_migration`**（复用 recovery_blocked，SHALL NOT 发明「人工隔离态」新 status;保留 pid/create_time/generation/instance/containment 待人工）;
                 进程树未确认清理前 SHALL 保持该 recovery_blocked、SHALL NOT 假装迁移完成、SHALL NOT 建 child。
...
- **THEN** SHALL 先 generation/attempt fencing → kill/containment 确认完整进程树退出 → attempt 落**已定义终态 `orphaned`** + execution 落**已定义终态 `recovery_blocked(blocked_reason=null_conversation_migration)`**（复用现有状态词、SHALL NOT 发明「迁移专用终态」「人工隔离态」新 status）;进程树未确认清理前 activate SHALL fail-closed（不假装迁移完成、不放任幽灵运行）
...
#### Scenario: NULL 迁移出口用普通 successor 不挂 superseded_from（第十四轮 P1-2）
- **WHEN** 一个 `recovery_blocked(null_conversation_migration)` 的 NULL execution 经人工迁移到新独立 conversation 后创建承接执行
- **THEN** 该承接执行是**普通 successor**（新 execution、独立 migration lineage/audit），SHALL NOT `superseded_from=NULL 父`——因 recovery chain SHALL scope 一致（父子同 conversation），而父 `conversation_id IS NULL`、child 为新 conversation，挂 `superseded_from` 会构成跨 conversation 错链被 P1-D chain 完整性校验拒绝;NULL 父保持 `recovery_blocked` 终态不变，任务完成度由 task 级 active execution 聚合（普通 successor 参与）
