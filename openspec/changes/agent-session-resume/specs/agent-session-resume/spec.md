...
- **🔴 鉴权 SHALL 校验资源级迁移权限（第十九轮 P1-5，不止有 actor 记审计）**：仅记录 `actor` 不足以防跨空间消息迁移。SHALL 校验：① 源 task 与目标 conversation **同租户/同 space**（SHALL NOT 跨租户/跨 space 迁移消息）;② `actor` 对**源 task 有管理权限** AND 对**目标 conversation 有写入权限**;③ 若 `target_conversation` 为**指定已有 conversation**（非新建），SHALL 额外确认它**未被其他 task 占用**（不违反 task:conversation 1:1）且上下文允许合并;不满足任一项返回 403（越权）或 409（目标被占用），并记审计。
- **请求体**：`{migration_token, target_conversation(新建或指定的目标 conversation), message_split(可审计的消息切分规格), expected_task_version, source_message_watermark, expected_cleanup_version, reason?}`——`migration_token` 为请求幂等键;切分规格 SHALL 可审计、无法可靠判定时服务端拒绝并保持 quarantine（不猜测迁移）。**🔴 三个快照版本字段绑定预览快照做 CAS（第十九轮 P1-5，防过期切分提交）**：操作者预览 message_split 到 POST 之间若源 task 新增消息、或 `process_cleanup_state` 变化，`expected_task_version`/`source_message_watermark`/`expected_cleanup_version` 与当前值不符时 SHALL 返回 **409 `snapshot_drift`** 要求重新预览，SHALL NOT 按过期切分提交（否则新增消息可能被漏迁或错切）。
...
- **幂等/冲突**：同 `migration_token` 重试返回同一 successor + `idempotent_replay=true`;不同 token·payload 一致返回已存在 successor + `idempotent_replay=true`;**不同 token·payload 不同**（不同目标 conversation/切分意图）返回 **409 `already_resolved`**（附赢家 successor + 其 `canonical_payload_hash`）;父非 `recovery_blocked(null_conversation_migration)` 返回 409;`process_cleanup_state=unconfirmed` 返回 409「待清理」;`expected_task_version`/`source_message_watermark`/`expected_cleanup_version` 与当前不符返回 409 `snapshot_drift`（第十九轮 P1-5）。
...
- **THEN** 迁移事务建普通 successor（`migration_from_execution_id=父`），响应返 `{migration_successor_id, target_conversation_id, resolution_kind:'migration', idempotent_replay:false}`;resolved cache 同事务写 `resolution_kind='migration'` + successor 指针，`GET /executions/{parent_id}` 立即返回 `resumed`（migration 类认 `migration_from_execution_id`，不再显 unresolved）;父流已封闭不追加事件
...
- **WHEN** `actor` 对源 task 无管理权限、或目标 conversation 与源 task 不同租户/space、或指定的目标 conversation 已被其他 task 占用
- **THEN** 分别返回 403（无源 task 管理权限/无目标写权限）、403（跨租户/跨 space 迁移）、409（目标 conversation 被占用违反 1:1），均记审计、不迁移任何消息;SHALL NOT 仅凭「请求带 actor」就放行跨空间消息迁移

#### Scenario: 预览快照漂移后 /migrate 拒绝过期切分（第十九轮 P1-5）
- **WHEN** 操作者预览 message_split 后、POST /migrate 之前，源 task 新增了消息（`source_message_watermark` 变化）或 `process_cleanup_state` 变化（`expected_cleanup_version` 不符）
- **THEN** 服务端比对 `expected_task_version`/`source_message_watermark`/`expected_cleanup_version` 与当前值不符，返回 409 `snapshot_drift` 要求重新预览;SHALL NOT 按过期切分提交（否则新增消息被漏迁或错切）;重新预览取最新快照后再 POST 才放行
