...
3. queued  NULL → 移出候选集 + **execution 落已定义终态 `recovery_blocked(blocked_reason=null_conversation_migration)`**（用户第十五轮拍板三态统一落 recovery_blocked、SHALL NOT 用无确定 run_queue status 的「标记 quarantine」概念——否则 execution 仍 queued 则 active NULL count 无法归零、activate 永久阻塞）;queued 无 attempt，`final_attempt_id` 保持 NULL（合法，见下方终态指针矩阵）;保留原始 pending intent 供人工迁移，不丢。
4. claimed NULL → attempt 落 `abandoned`（无残留进程、CLI 从未起）+ retire session owner（epoch+1）+ 清 `run_queue.current_attempt_id` + **execution 落已定义终态 `recovery_blocked(blocked_reason=null_conversation_migration)`**;`final_attempt_id` 指向该 `abandoned` attempt（见矩阵）;保留 pending intent。
5. running NULL → **两阶段持久化清理协议（第十七轮 P0，kill/containment/OS 探活不在 SQLite 事务内，故 DB 落态与进程清理 SHALL 拆两个事务、可崩溃恢复）**。execution 从一开始即 `recovery_blocked(blocked_reason=null_conversation_migration)`，进程清理用正交**可变**字段 `process_cleanup_state(unconfirmed|confirmed)` + `cleanup_version`（CAS 用）表达（SHALL NOT 用 `process_not_confirmed_dead` 作 blocked_reason——否则通用恢复会建不可执行的 `superseded_from=NULL 父` recovery child、迁移身份丢失）。**attempt 首次即落 `orphaned` 并永久保持（第十七轮 P0：`orphaned` 是 attempt 终态、不可逆，SHALL NOT 在确认退出后改写为 `abandoned`——否则破坏 attempt 终态不可逆铁律与 late_event_after_terminal 不变量）**；「进程是否已确认退出」由**可变的 `process_cleanup_state` 表达、非改写 attempt 终态**，展示层「已清理」用派生态呈现（`orphaned` + `cleanup_state=confirmed`），SHALL NOT `UPDATE task_runs.status`：

   **事务 A `quarantine_null_execution()`（首次终态）**：CAS source `running` → attempt `orphaned`（终态、永久）+ execution `recovery_blocked(null_conversation_migration)` + `process_cleanup_state=unconfirmed` + `cleanup_version=0` + owner retire + current/final pointer（final 指该 orphaned attempt）+ `terminal(recovery_blocked)` event + audit，单提交。
   **事务外**：kill/containment + 校验完整进程树退出 + `pid_create_time` + generation/instance 证据。
   **事务 B `confirm_null_process_cleanup()`（清理确认，第十七轮 P0）**：CAS `execution 仍 recovery_blocked(null_conversation_migration)` AND `process_cleanup_state=unconfirmed` AND `cleanup_version=expected` → 置 `process_cleanup_state=confirmed` + `cleanup_version+1` + 写 `cleanup_confirmed_at/by/evidence_hash` + audit；**attempt 永久保持 `orphaned` 不改终态、SHALL NOT 向已封闭的父 SSE 流写第二 terminal 事件**（清理证据进 audit/派生态）；单提交。并发人工确认与后台巡检 SHALL 由该 CAS 保证只一个胜出；`pid` 复用但 `create_time` 不同的旧证据 SHALL NOT 确认清理。
   **migration gate**：`null migration 迁移出口`（见下）SHALL 仅在 `process_cleanup_state=confirmed`（running 来源）或 `source=queued/claimed`（从未起进程）时放行；`unconfirmed` 时 SHALL 拒绝迁移、只允许继续清理。
   **崩溃恢复**：事务 A 提交后、kill 未执行即崩溃 → 重启巡检发现 `unconfirmed` 继续清理、不建 successor；kill 完成、事务 B 前崩溃 → 重启按 `pid+create_time` 指纹重确认后 CAS `confirmed`（幂等）。

   execution 全程保持 `recovery_blocked(null_conversation_migration)` 终态不变、SHALL NOT 假装迁移完成、SHALL NOT 走 generic `resume_blocked_execution()` 建 `superseded_from` NULL recovery child;是否可回队由 execution status 决定（recovery_blocked=不回队）。
6. 校验 activate readiness = 三段全零（第十四轮 P1-2，仅 active NULL count 不足以表达 fail-closed）：
   ├ active NULL count（queued/claimed/running 且 conversation_id IS NULL）= 0
   ├ unresolved NULL migration blocker count（execution `recovery_blocked(null_conversation_migration)` 且 `process_cleanup_state=unconfirmed`，第十六轮 P1-C）= 0
   └ surviving NULL process-tree count（残留 NULL 旧 CLI 进程树未确认退出）= 0
7. 任一非 0 → activate/readiness fail-closed，不开新状态写入与三层硬门。
8. 三段全零通过后才开启新状态写入和三层硬门。
```

**为何 `active NULL count=0` 单独不够（第十四轮 P1-2）**：running NULL 一旦落 `recovery_blocked`（非 active 三态），`active NULL count` 即归零，但其旧进程树可能仍未确认退出——只看 active count 会误判可 activate、放任幽灵运行。故 readiness SHALL 同时校验「未解决的 `null_conversation_migration` blocker=0」与「存活 NULL 进程树=0」，任一非零即 fail-closed。

**三态统一落 `recovery_blocked(null_conversation_migration)` 的终态指针矩阵（第十五轮 P1-2，用户拍板三态统一、SHALL NOT 用无确定 status 的「标记」概念）**——三态 execution 均落已定义终态 `recovery_blocked(null_conversation_migration)`（使 active NULL count 能归零），attempt/`final_attempt_id` 按来源分：

| 来源 | attempt 终态 | execution 终态 | final_attempt_id | pending intent |
|---|---|---|---|---|
| queued NULL | 无（从未 claim、无 attempt） | `recovery_blocked(null_conversation_migration)` | **NULL 合法**（无定局 attempt，消费者 SHALL 容忍 recovery_blocked 且 final=NULL） | 保留 |
| claimed NULL | `abandoned`（`abandon_stage=prelaunch`、`abandon_reason=null_conversation_migration`；CLI 未起、无残留进程） | `recovery_blocked(null_conversation_migration)` | 指向该 `abandoned` attempt | 保留 |
| running NULL（**第十七轮 P0 两阶段、attempt 恒 `orphaned` 不改写**） | `orphaned`（首次即落、永久终态） | `recovery_blocked(null_conversation_migration)` + `process_cleanup_state`（`unconfirmed`→已确认退出后经 `confirm_null_process_cleanup()` CAS 翻 `confirmed`，**attempt 不变**） | 指向该 `orphaned` attempt | 保留 |

消费者矩阵 SHALL 覆盖「`recovery_blocked(null_conversation_migration)` 且 `final_attempt_id` 可能为 NULL（queued 来源）」——dead-letter/Runtime/progress/失败率/partial index/claim 候选均按 recovery_blocked 终态识别、不进 active、不计业务失败率、不被 claim。`quarantined_from_conversation_id` lineage 列仍写入（记录原 NULL 来源供人工追溯），但 execution 的**权威状态是 `recovery_blocked` 终态**、非独立「quarantine 标记」。`pending`/`session` 指针处置 SHALL 定义（queued 保 pending intent、claimed/running retire owner）。**人工迁移出口 SHALL 走普通 successor、不挂 `superseded_from`（第十四轮 P1-2，避免与 P1-D recovery chain scope 一致冲突）**：NULL task 迁到新独立 conversation 后创建的是一个**普通 successor**（新 execution、独立 migration lineage/audit、SHALL NOT `superseded_from=NULL 父`）——因 recovery chain SHALL scope 一致（父子同 conversation），而此处父 `conversation_id IS NULL`、child 为新 conversation，挂 `superseded_from` 会构成跨 conversation 错链;迁移后按「可执行 task 必须拥有非 NULL conversation」Requirement 的人工出口正常执行。SHALL NOT 只写产品概念而不定字段。**收尾后旧 NULL 行 SHALL NOT 再被任何 Worker claim/finalize/写平台**（三层硬门 + fencing 双保险）。

#### Scenario: queued NULL 行 activate 前移出候选并落 recovery_blocked（第十五轮 P1-2）
- **WHEN** activate 前存在 `conversation_id IS NULL` 的 `queued` 行
- **THEN** 移出候选集 + execution 落已定义终态 `recovery_blocked(null_conversation_migration)`（使 active NULL count 能归零、SHALL NOT 停留在 queued），`final_attempt_id` 保持 NULL（无 attempt，合法）;保留原始 intent 供人工迁移;因 CLI 从未起、无残留进程，无需 fencing/kill;SHALL NOT 丢失该 intent

#### Scenario: claimed NULL 行 activate 前 abandon 并落 recovery_blocked（第十五轮 P1-2）
- **WHEN** activate 前存在 `conversation_id IS NULL` 的 `claimed` 行（已建 attempt、持 claim lease、CLI 未起）
- **THEN** attempt 落 `abandoned`、retire session owner（epoch+1）、清 `run_queue.current_attempt_id`、execution 落已定义终态 `recovery_blocked(null_conversation_migration)`、`final_attempt_id` 指向该 abandoned attempt;SHALL NOT 只标记业务隔离而留 attempt/owner 悬挂、SHALL NOT 停留在 claimed 使 active count 无法归零

#### Scenario: running NULL 事务 A 首次落 orphaned + unconfirmed（第十七轮 P0）
- **WHEN** activate 前存在 `conversation_id IS NULL` 的 `running` 行，其 CLI 仍在执行、进程树未确认退出
- **THEN** 事务 A `quarantine_null_execution()`：attempt 落 `orphaned`（终态、**永久**）+ execution `recovery_blocked(null_conversation_migration)` + `process_cleanup_state=unconfirmed` + `cleanup_version=0` + owner retire + `final_attempt_id` 指该 orphaned + 恰好一个 `terminal(recovery_blocked)` event;activate SHALL fail-closed;**出口只允许继续清理进程、SHALL NOT 建任何 successor、SHALL NOT 走 generic resume 建 NULL recovery child**

#### Scenario: running NULL 事务 B 确认清理翻 confirmed、attempt 不改写（第十七轮 P0）
- **WHEN** 进程树经 kill/containment 已确认完整退出（`pid+create_time` 指纹核对一致）
- **THEN** 事务 B `confirm_null_process_cleanup()` CAS（`execution 仍 recovery_blocked(null_conversation_migration)` AND `process_cleanup_state=unconfirmed` AND `cleanup_version=expected`）→ `process_cleanup_state=confirmed` + `cleanup_version+1` + 写 `cleanup_confirmed_at/by/evidence_hash`;**attempt 永久保持 `orphaned` 不改写为 `abandoned`（第十七轮 P0：终态不可逆）**、SHALL NOT 向已封闭父 SSE 流写第二 terminal;展示「已清理」用派生态（`orphaned`+`cleanup_state=confirmed`）呈现;之后迁移出口放行（进 null migration 事务建新 conversation 普通 successor）

#### Scenario: 两阶段清理并发确认与崩溃恢复（第十七轮 P0）
- **WHEN** 后台巡检与人工同时确认清理;或事务 A 提交后 kill 未执行即崩溃;或 kill 完成、事务 B 前崩溃;或 `pid` 复用但 `create_time` 不同
- **THEN** `confirm_null_process_cleanup()` 的 `cleanup_version` CAS 保证并发确认只一个胜出（attempt/final pointer 不变）;事务 A 后崩溃→重启巡检发现 `unconfirmed` 继续清理、不建 successor;事务 B 前崩溃→重启按 `pid+create_time` 指纹重确认后幂等 CAS `confirmed`;`create_time` 不匹配的旧证据 SHALL NOT 确认清理

#### Scenario: NULL running 恢复出口禁 generic recovery child（第十六轮 P1-C）
- **WHEN** 一个 `recovery_blocked(null_conversation_migration)` 的 execution（无论 process_cleanup_state）被尝试用通用 `resume_blocked_execution()` 恢复
- **THEN** SHALL 拒绝——`null_conversation_migration` 的恢复出口唯一是 null migration 事务（新 conversation 普通 successor），SHALL NOT 建 `superseded_from=NULL 父` 的 recovery child（父 conversation=NULL、child 仍 NULL 会被三层硬门拒绝、迁移身份丢失）;`process_cleanup_state=unconfirmed` 时连 migration 事务也拒绝（须先确认清理）、只允许继续清理

#### Scenario: activate readiness 三段全零才放行（第十四轮 P1-2）
- **WHEN** 三态收尾进行中，检查 activate readiness
- **THEN** SHALL 同时校验三段——① active NULL count（queued/claimed/running 且 conversation_id IS NULL）= 0;② unresolved NULL migration blocker count（execution `recovery_blocked(null_conversation_migration)` 且 `process_cleanup_state=unconfirmed`）= 0;③ surviving NULL process-tree count（残留 NULL 旧 CLI 进程树未确认退出）= 0;任一非 0 即 activate/readiness fail-closed，仅三段全零才放行
- **AND** running NULL 已落 `recovery_blocked` 使 active count 归零、但 `process_cleanup_state=unconfirmed`（进程树未确认退出）时，②/③ 非零仍 fail-closed（SHALL NOT 只凭 active count=0 误判可 activate）
...
- **WHEN** 人工恢复：建新 conversation + 消息切分 + 更新 `tasks.conversation_id` + 转移 pending intent + 建普通 successor + 写 migration request/lineage/audit（第十五轮 P1-4 完整事务集）
- **THEN** 上述动作 SHALL 全事务提交或全回滚，SHALL NOT 出现「新 conversation 已建但消息未迁」「消息迁一半」等半提交;迁移后完整性探针核对无孤儿/串档
- **AND** **旧 NULL execution 的 `run_queue.conversation_id` 快照 SHALL 永久保持 NULL**（第十五轮 P1-4，与 [platform-graceful-restart] execution conversation 快照永久不可变一致）——「修正 run 归属」SHALL 只指新建的 migration successor（`tasks.conversation_id` 更新 + 新 successor 带新 conversation），SHALL NOT 改写历史 NULL execution 的 conversation 快照

#### Scenario: NULL 迁移出口用普通 successor 不挂 superseded_from、带迁移因果键（第十四轮 P1-2 + 第十五轮 P1-4）
- **WHEN** 一个 `recovery_blocked(null_conversation_migration)` 的 NULL execution 经人工迁移到新独立 conversation 后创建承接执行
- **THEN** 该承接执行是**普通 successor**（新 execution、开启自己的新 recovery chain `recovery_chain_id=自身 id`/`recovery_attempt=0`），SHALL NOT `superseded_from=NULL 父`——因 recovery chain SHALL scope 一致（父子同 conversation），父 `conversation_id IS NULL`、child 为新 conversation，挂 `superseded_from` 会构成跨 conversation 错链被 P1-D 校验拒绝;**迁移因果 SHALL 用独立键 `run_queue.migration_from_execution_id`（指向旧 NULL execution）+ `UNIQUE(migration_from_execution_id) WHERE migration_from_execution_id IS NOT NULL`（第十五轮 P1-4，镜像 `superseded_from` 的一父一子基数）**，使全因果叶子聚合能确定哪个 successor 是迁移承接;NULL 父保持 `recovery_blocked` 终态不变，任务完成度由全因果叶子优先级聚合（migration successor 作为叶子参与）

#### Scenario: NULL migration 请求幂等（第十五轮 P1-4）
- **WHEN** 人工迁移响应丢失后用**同一** `request_token` 重试;或不同 token 并发迁移同一 NULL 父;或同 token 不同 payload
- **THEN** SHALL 用 `null_migration_requests` 表 `{parent_execution_id, request_token, raw_payload_hash(仅审计、可选), canonical_payload_hash(一切相等判断只比此字段), child_execution_id, target_conversation_id, actor, created_at, outcome(winner|idempotent_replay|rejected_already_resolved)}` + `UNIQUE(parent_execution_id, request_token)` 保证（第十九轮 P1-2 统一 canonical_payload_hash 口径，见 PGR canonical 算法）：同 token 重试返回同一 migration successor;**不同 token 并发经 `UNIQUE(migration_from_execution_id)` 冲突后 SHALL 比对 `canonical_payload_hash`——一致读回已存在 successor（`idempotent_replay=true`）、SHALL NOT 建第二个;不一致（不同目标 conversation/不同切分意图）返回 409 `already_resolved`（附赢家 successor + 其 `canonical_payload_hash`，第十七轮 P1-D，与 PGR recovery request 同口径）、SHALL NOT 静默映射不同意图到赢家 successor**;同 token 不同 payload 返回 409;**active partial unique index 只挡同一时刻并发、SHALL NOT 依赖它防「首个 successor 完成后再次重放建第二个」**——持久因果键 `UNIQUE(migration_from_execution_id)` 才是防重放的真相源

**🔴 NULL migration 人工出口 SHALL 有完整 HTTP API 契约（第十八轮，补「只写内部事务、无对外 endpoint」）**：NULL migration 不复用 recovery 的 `POST /executions/{id}/resume`（那建 `superseded_from` recovery child、语义不符），SHALL 有独立端点：
- **端点**：`POST /executions/{parent_id}/migrate`（`parent_id`=`recovery_blocked(null_conversation_migration)` 的 NULL execution）。
- **🔴 鉴权 SHALL 映射当前 schema、不臆造无数据结构的 tenant/space（第十九轮 P1-5 + 第二十轮 P1-5：现网 `tasks`/`conversations` 均有 `project_id NOT NULL REFERENCES projects(id)`，权限模型只有单管理员 `require_admin` + 匿名只读，无 tenant/space/membership 表）**：SHALL 校验——① 写入口继续 `require_admin`（沿用现有权限模型）;② **`source_task.project_id == target_conversation.project_id`**（同项目，用现网 `project_id` 列实现「同空间」语义，SHALL NOT 跨 project 迁移消息）;③ 若 `target_conversation` 为**指定已有 conversation**（非新建），SHALL 额外确认它**未被其他 task 占用**（不违反 task:conversation 1:1）且上下文允许合并;不满足 ② 返回 403、③ 返回 409（目标被占用），并记审计。**若未来确需项目成员/角色级 ACL，SHALL 另立授权模型 change 增 project membership/role schema，SHALL NOT 在本 change 只写 tenant/space 概念却无数据结构**（第二十轮 P1-5）。
- **请求体**：`{migration_token, target_conversation(新建或指定的目标 conversation), message_split(可审计的消息切分规格), expected_task_row_version, source_message_watermark, expected_cleanup_version, reason?}`——`migration_token` 为请求幂等键;切分规格 SHALL 可审计、无法可靠判定时服务端拒绝并保持 quarantine（不猜测迁移）。**🔴 快照 CAS 字段 SHALL 映射可执行 schema（第十九轮 P1-5 + 第二十轮 P1-5：现网 `tasks` 只有 `updated_at` 无单调 version）**：
  - **`expected_task_row_version`**：SHALL 新增 `tasks.row_version INTEGER NOT NULL DEFAULT 0` 单调列（每次相关修改递增），预览时取值、POST 时 CAS 比对（SHALL NOT 依赖 `updated_at` 字符串做并发判据）。**🔴 完整 writer 矩阵 SHALL 定义（第二十一轮 P1-3，否则任一 writer 漏增 → 旧 preview 的 `expected_task_row_version` 误放行、基于过期 task 快照切分）**：① **「相关修改」定义** = 任何影响迁移预览正确性的列变化（至少 `conversation_id`、`status`、标题/负责人/归属等被 migration 或 preview 读取的字段;纯 `updated_at` 触碰不单独算）;② **所有 `UPDATE tasks` 路径 SHALL 递增 `row_version`**，覆盖仓库全部写入口：`progress.py`（自动 reviewing/done 状态流转）、`agent_cli.py`（Agent 状态更新）、`routes/runs.py`（重跑回写）、`routes/tasks.py`（管理员编辑与状态更新）、数据迁移脚本、以及 NULL migration 自身更新 `tasks.conversation_id`;③ **递增 SHALL 由 DB `BEFORE UPDATE` 触发器或统一 repository 层保证**（不依赖每个调用点记得手写 `row_version+1`，防遗漏）;④ **迁移事务 SHALL 用单语句原子 CAS、SHALL NOT 先 `SELECT` version 再无条件 `UPDATE`**：`UPDATE tasks SET conversation_id=?, row_version=row_version+1, ... WHERE id=? AND row_version=:expected AND conversation_id IS NULL`——影响行数=0 即视为快照漂移返 409 `snapshot_drift`;⑤ 探针 `task_row_version_writer_probe`（静态扫描仓库全部 `UPDATE tasks` 路径均经统一 repository/触发器递增 row_version、无裸 UPDATE 绕过）+ `migrate_row_version_cas_probe`（迁移用单语句 CAS、预览后任一 writer 递增即 409、不先查后改）。
  - **`source_message_watermark`**：= 预览时 `MAX(messages.id WHERE conversation_id=源)`，迁移事务内复核，不符即漂移。
  - **`expected_cleanup_version`**：**queued/claimed 来源**（无 running 残留进程）该字段固定为 `0`、可选携带;**running 来源**（两阶段清理）SHALL 精确匹配当前 `cleanup_version`（防预览后 `confirm_null_process_cleanup()` 又推进）。
  - 三者任一与当前不符 SHALL 返回 **409 `snapshot_drift`** 要求重新预览，SHALL NOT 按过期切分提交（否则新增消息漏迁/错切）。
- **前置校验**：① 父 execution 为 `recovery_blocked AND blocked_reason=null_conversation_migration`;② **`process_cleanup_state` 非 `unconfirmed`**（running NULL 未确认清理时 SHALL 拒绝迁移、返回「待清理」，见第十七轮 P0）;③ 目标 conversation 满足 1:1 约束。
- **成功响应**（201/200）：`{parent_execution_id, migration_successor_id, target_conversation_id, resolution_kind: 'migration', idempotent_replay: bool}`——调用端据 `migration_successor_id` 订阅新 successor 流。
- **幂等/冲突**：同 `migration_token` 重试返回同一 successor + `idempotent_replay=true`;不同 token·payload 一致返回已存在 successor + `idempotent_replay=true`;**不同 token·payload 不同**（不同目标 conversation/切分意图）返回 **409 `already_resolved`**（附赢家 successor + 其 `canonical_payload_hash`）;父非 `recovery_blocked(null_conversation_migration)` 返回 409;`process_cleanup_state=unconfirmed` 返回 409「待清理」;`expected_task_row_version`/`source_message_watermark`/`expected_cleanup_version` 与当前不符返回 409 `snapshot_drift`（第十九轮 P1-5 + 第二十轮 P1-5 映射 `tasks.row_version`）;`source_task.project_id != target_conversation.project_id` 返回 403（第二十轮 P1-5）。
- **事务**：走 [platform-graceful-restart] 的迁移事务（建新 conversation + 消息切分 + 更新 `tasks.conversation_id` + 转移 pending intent + 建普通 successor（`migration_from_execution_id=父`）+ 写 `null_migration_requests`/lineage/audit），全提交或全回滚。**resolved 判定按 [platform-graceful-restart] 的「二选一」口径（第十九轮 P1-5）**：默认纯派生（`EXISTS(successor WHERE migration_from_execution_id=父)`）、不写 cache;仅当全局选定「完整 schema cache」口径时，才在**同一迁移事务**内写 `resolution_successor_id`/`resolution_kind='migration'` 冗余列——SHALL NOT 在派生口径下要求迁移必写 cache（消解与 PGR「可选 cache」的矛盾）。

#### Scenario: POST /migrate 返回 migration_successor_id 且 cache 一致
- **WHEN** 人工对 `recovery_blocked(null_conversation_migration)` 且 `process_cleanup_state≠unconfirmed` 的 NULL execution 调 `POST /executions/{parent_id}/migrate`，前置满足
- **THEN** 迁移事务建普通 successor（`migration_from_execution_id=父`），响应返 `{migration_successor_id, target_conversation_id, resolution_kind:'migration', idempotent_replay:false}`;**`GET /executions/{parent_id}` 立即返回 `resumed` 恒由因果键纯派生判定 `EXISTS(successor WHERE migration_from_execution_id=父)`（第二十轮 P1-3：因果键为 resolved 唯一权威）**，默认不写 cache;**仅当全局启用「完整 schema cache」口径时**才在同一迁移事务内写 `run_queue.resolution_successor_id`（cache 是因果键的一致副本、非第二真相源），SHALL NOT 在默认纯派生口径下要求迁移必写 cache;父流已封闭不追加事件

#### Scenario: POST /migrate 前置/冲突返回结构化拒绝
- **WHEN** 对 `process_cleanup_state=unconfirmed` 的父、或非 null-migration 父、或不同 token·不同目标 conversation 并发调 `/migrate`
- **THEN** 分别返回 409「待清理」/409（父非 null-migration）/409 `already_resolved`（附赢家 successor），不建第二个 successor、不改父状态、不静默映射不同意图

#### Scenario: /migrate 越权与跨空间迁移被拒（第十九轮 P1-5）
- **WHEN** 非管理员调用（未过 `require_admin`）、或 `source_task.project_id != target_conversation.project_id`（跨项目迁移）、或指定的目标 conversation 已被其他 task 占用
- **THEN** 分别返回 401/403（非管理员）、403（跨 project 迁移，用现网 `project_id` 列判定）、409（目标 conversation 被占用违反 1:1），均记审计、不迁移任何消息;SHALL NOT 仅凭「请求带 actor」就放行跨 project 消息迁移;项目成员/角色级细粒度 ACL 若需要走独立授权 change、不在本 change 臆造无 schema 的 tenant/space

#### Scenario: 预览快照漂移后 /migrate 拒绝过期切分（第十九轮 P1-5 + 第二十一轮 P0-1）
- **WHEN** 操作者预览 message_split 后、POST /migrate 之前，源 task 被修改（`tasks.row_version` 递增）、源 conversation 新增消息（`source_message_watermark`=该 conversation 的 `MAX(message_seq)` 变化）、或 `process_cleanup_state` 经 `confirm_null_process_cleanup()` 推进（`cleanup_version` 变化）
- **THEN** 服务端在迁移事务内 CAS 比对 `expected_task_row_version`（对 `tasks.row_version`）/`source_message_watermark`（对当前该 conversation 的 `MAX(message_seq)`，conversation 序号非全局 id）/`expected_cleanup_version`（running 来源精确匹配、queued/claimed 来源固定 0）与当前值不符，返回 409 `snapshot_drift` 要求重新预览;SHALL NOT 按过期切分提交（否则新增消息被漏迁或错切）;重新预览取最新快照后再 POST 才放行

#### Scenario: migration 与并发消息写入被同一 conversation 锁串行、无 phantom 漏迁（第二十一轮 P0-1）
- **WHEN** migration 事务读 `source_message_watermark` 后、提交前，另有并发消息写入同一 source conversation
- **THEN** migration 事务 SHALL 在切分前锁定同一 source conversation 行（取当前 `MAX(message_seq)` 复核 `source_message_watermark`）——并发消息写入因走同一 conversation 行锁被串行化：要么在 migration 锁前提交（其 seq 已纳入 watermark、被正确切分），要么在 migration 锁后才分配 seq（migration 持锁期间它阻塞）；migration 提交后若发现 watermark 已变则返 409 重试。**SHALL NOT 出现「watermark 检查与迁移提交之间的 phantom insert 拿到更小 seq 却被漏切」**（全局 `messages.id` 下 phantom 可拿到检查点之后才提交的较小 id 逃过 watermark，故 SHALL 用 conversation 锁 + `message_seq` 而非全局 id）

### Requirement: NULL 隔离/迁移的独立终态事务、lineage 完整性与输家映射（第十六轮 P1-D）

**1. 独立 `quarantine_null_execution()` 终态事务**：queued NULL 无 attempt，SHALL NOT 机械复用「以定局 attempt 为中心」的 `finish_execution()`。三态隔离 SHALL 走独立事务 `quarantine_null_execution()`，在**同一提交**内完成：① source status CAS（queued/claimed/running → 隔离）;② attempt 状态（claimed→abandoned;**running 恒 →orphaned（第十七轮 P0：不分 confirmed/unconfirmed、attempt 首次即永久终态，进程是否确认退出由正交的 `process_cleanup_state` 表达、SHALL NOT 用 attempt 状态区分）**;queued 无 attempt 跳过）;③ owner retire + current/final pointer（claimed/running 指定局 attempt、queued final=NULL）;④ pending intent 保留;⑤ execution 落 `recovery_blocked(null_conversation_migration)` + `process_cleanup_state`（running 首次恒 `unconfirmed`、queued/claimed 无残留进程恒 `confirmed`）;⑥ `terminal(recovery_blocked)` event（遵全局 id 顺序）;⑦ migration audit。任一 CAS/唯一约束失败整体回滚。**SHALL NOT 让 execution 已终态但 SSE 无 terminal、或 owner/pointer/status 分开提交**（半提交防护同 `finish_execution()`）。

#### Scenario: queued NULL 隔离有独立事务和 terminal event
- **WHEN** queued NULL 行经 `quarantine_null_execution()` 隔离，在 status CAS / execution recovery_blocked / terminal event / audit 各写点注入故障
- **THEN** 整体提交或整体回滚，SHALL NOT 出现「execution 已 recovery_blocked 但无 terminal event」「status 改了但 pending 未保留」半提交;成功后恰好一个 `terminal(recovery_blocked)` event 在 execution 事件流

**2. migration lineage 完整性**（`migration_from_execution_id` 不止 UNIQUE）SHALL 补：① **parent execution 存在**（FK 等价校验，不悬空）;② **parent 是 `recovery_blocked(null_conversation_migration)` 上下文**（SHALL NOT 对非 NULL-migration 父建 migration successor）;③ **parent/child `task_id`、`agent_slug` 一致，conversation 按设计从 NULL 变为新值**（唯一允许变的维度）;④ **`migration_from_execution_id` 创建后不可变**;⑤ **删除 parent/child 有守卫或整组归档**（SHALL NOT 单独删留悬空）;⑥ **禁 self-reference** `child.id != migration_from_execution_id`;⑦ integrity scan 扫 dangling、cross-task、mutable lineage。校验与 successor insert 同一事务原子（非先查后插）。

#### Scenario: migration lineage 完整性校验
- **WHEN** 构造悬空 parent / 非 null-migration 父 / 跨 task 或跨 agent 的 migration successor / 自引用 / 事后修改 migration_from_execution_id
- **THEN** 分别被中心化 migration 事务 + integrity 探针的 FK 等价校验拒绝（parent 存在且为 null_conversation_migration、task_id/agent_slug 一致、conversation 仅从 NULL 变新值、字段不可变、禁自引用）;删除仍被引用的 parent 被守卫拒绝或 parent/child/request 原子归档

**3. `null_migration_requests` 不同 token 输家按 payload 分两种结局**（镜像 recovery request winner/loser，第十六轮 P1-D + 第十七轮 P1-D;第十九轮 P1-2 比较字段统一 `canonical_payload_hash`）：赢家在迁移事务内写 `{本 token, canonical_payload_hash, outcome=winner → 新 successor}`;不同 token 输家 SHALL 在**失败事务回滚后的独立事务**里先比对本请求与赢家的 `canonical_payload_hash`——**一致**只写 `{本 token, canonical_payload_hash, outcome=idempotent_replay → existing successor}` 映射、返回 `idempotent_replay=true`;**不一致**（不同目标 conversation/不同切分意图）只写 `{本 token, canonical_payload_hash, outcome=rejected_already_resolved}` 审计、返回 409 `already_resolved`（附赢家 successor）——均不重复迁移副作用，否则输家响应再丢无法按 token 稳定重放、不同意图会被静默吞掉。

#### Scenario: 不同 token·payload 一致输家持久映射 existing successor
- **WHEN** 不同 token 并发迁移同一 NULL 父、payload 一致，输家因 `UNIQUE(migration_from_execution_id)` 冲突
- **THEN** 输家先回滚失败的 successor-insert 事务，再独立事务比对本请求与赢家 `canonical_payload_hash` 一致、只写 `null_migration_requests(本 token, canonical_payload_hash, outcome=idempotent_replay → existing successor)` 映射、返回 `idempotent_replay=true`;SHALL NOT 重复建 successor/重复迁移消息/重复副作用;此后该 token 重试稳定返回同一 successor

#### Scenario: 不同 token·payload 不同输家返回 409 already_resolved（第十七轮 P1-D）
- **WHEN** 不同 token 并发迁移同一 NULL 父、payload 不同（如迁向不同目标 conversation），其一先胜出
- **THEN** 输家 `UNIQUE(migration_from_execution_id)` 冲突后比对本请求与赢家 `canonical_payload_hash` 不一致，独立事务只写 `null_migration_requests(本 token, canonical_payload_hash, outcome=rejected_already_resolved)` 审计、返回 409 `already_resolved`（附赢家 successor）;SHALL NOT 把输家的不同迁移意图静默映射到赢家 successor、SHALL NOT 建第二个 successor
...
**🔴 消息水位 SHALL 用 conversation 提交顺序安全序号 `conversations.next_message_seq`（conversation 行锁原子分配），SHALL NOT 用 `messages.id`/全局自增主键作水位（第二十一轮 P0-1）**：根因同 SSE——PostgreSQL identity 在 INSERT 时分配、不等提交，一条先拿到小 `messages.id` 但迟提交的用户消息，会落在 reader 已推进的 `committed` 水位之后，因 `id <= committed` 永久不再回灌，直接破坏 at-least-once。改为每条消息写入时在事务内锁其 conversation 行（PG `SELECT next_message_seq FROM conversations WHERE id=:cid FOR UPDATE`、SQLite 过渡期靠单写事务）取 `message_seq`、写 message、`next_message_seq+=1` 同事务提交——**同 conversation 内消息 seq 无未提交空洞被越过**（conversation 行锁串行化了 seq 分配与提交）。下文所有「`messages.id` 水位/`MAX(messages.id)`」SHALL 按此 conversation 序号语义理解（`message_seq`），`committed_msg_id`/`planned_through_msg_id`/`batch_scan_end`/`source_message_watermark` 均 SHALL 是 conversation 内的 `message_seq` 而非全局 id。**SQLite 过渡兼容**：单写事务下全局 `messages.id` 无 late-commit 越位、可作 `message_seq` 的过渡实现，但契约真相源是 conversation 序号语义、迁 PostgreSQL 时 SHALL 按 conversation 行锁分配落地（见 [platform-concurrency-scaling] PG-first DAG，第二十一轮 P1-5）。

系统 SHALL 在复用 session 执行时只回灌「增量上下文」,而非全量历史。增量水位 SHALL 用 **2 字段两阶段**：`agent_sessions.committed_msg_id`（上次**成功**执行确认的水位，**只有 run 成功收尾才推进**）+ `task_runs.planned_through_msg_id`（本次构建 prompt 那刻的快照终点 = 当时该 conversation 的 `MAX(message_seq)`）。系统 **SHALL NOT** 用「执行完成时的 MAX(seq)」做单一水位（并发下会把执行期间别人新写的消息算进而漏话）。增量 = `messages WHERE conversation_id=? AND message_seq > committed_msg_id AND message_seq <= planned_through_msg_id AND 作者非本 agent`（参数化，`message_seq` 为 conversation 序号）：别人/人工在上次成功水位之后、本次快照之前说的话喂给本 Agent,本 Agent 自己的历史发言由 CLI session 记忆承载、不重喂。**不变量 = at-least-once**：崩溃/中断时 committed 未推进 → 续跑从同一起点重取 → 重复但不漏;prompt 构建后、CLI 接收前崩溃时 committed **SHALL NOT** 提前推进。**单次海量增量 SHALL 用连续前缀分批，SHALL NOT 用「保新丢旧」裁剪**：现状历史裁剪按保留最新、丢弃较早裁剪，若套在增量上会丢中段较早消息而 committed 仍推进到 planned_through → 中段永久漏喂，违反 at-least-once。增量 SHALL 从 committed 之后最旧一条起、按连续前缀截取本轮可喂量。**committed 水位 SHALL 定义为「原始消息扫描水位」`batch_scan_end`，SHALL NOT 定义为「最后一条实际拼入 prompt 的消息 id」**（Review 第四轮 P0-2）——后者在「尾部消息全是本 Agent 自产、被过滤」时会停在旧位置，使 `committed < planned` 恒成立、无限创建没有历史可投递的空 backlog successor 直到批次上限误转人工。正确语义：

```text
batch_scan_end = 本批已检查并完成取舍的最高原始 message_seq（conversation 序号，非全局 id）
eligible messages = 扫描区间内真正拼入 prompt 的非自产消息
成功后 committed_msg_id = batch_scan_end   （不是最后一条 eligible 的 seq）
```
...
- **THEN** 系统只把 `message_seq > committed_msg_id AND message_seq <= planned_through_msg_id` 且非本 agent 自产的新增 messages 拼进 prompt,不重复喂更早历史,也不重喂本 Agent 自己的旧发言

#### Scenario: 并发不漏话
- **WHEN** Agent A 执行期间,Agent B 在同一 conversation 写了发言（其 message_seq 大于 A 本次的 planned_through_msg_id）
- **THEN** B 的发言不在 A 本次增量内，但因 A 的 committed 只推进到本次成功水位（仍在 B 之前）,B 的发言在下次触发 A 时被正确纳入增量,不漏

#### Scenario: PostgreSQL 提交顺序：迟提交小 seq 用户消息不被漏灌（第二十一轮 P0-1）
- **WHEN** 同一 conversation 下 T1 先在事务内锁行分配 `message_seq=50` 但迟提交、T2 后到（因 conversation 行锁不能抢先拿 51 提交），Agent A 本轮 prompt 快照在 T1 提交前取 `planned_through_msg_id`
- **THEN** 因 `message_seq` 由 conversation 行锁分配、分配与提交被串行化，A 本轮快照 SHALL NOT 跨过未提交的 seq=50；T1 提交后 seq=50 落在下一轮 `committed < message_seq <= planned_through` 区间内被完整回灌，**SHALL NOT 出现「小 seq 用户消息迟提交、落在 committed 之后被永久漏灌」**（对比：全局 `messages.id` 在 INSERT 时分配不等提交，T2 可拿 51 先提交致 reader 推 committed 越过 50，故 SHALL NOT 用全局 id 作水位）
