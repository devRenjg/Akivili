...
- 新表 `agent_sessions(id, conversation_id, agent_slug, session_id, snapshot_msg_id, provider_id, backend, workdir, updated_at)`，唯一键 `(conversation_id, agent_slug)`。
- **同 (task, agent) 串行（`(conversation_id, agent_slug)` 上的 partial unique index）**：同一成员在同一 task 上**至多一个 queued/running run**。若该成员已有在跑/在排队 run，新触发**折叠进下一轮**而非并行起第二条。这样既防同一 Agent 对同一 task 重复并行做工，又让「增量边界」有确定的推进时机。
...
- **resume 落地判定**：claude 请求了 resume 但 stderr 出现 "no conversation found"、或输出的 session_id 与请求的不一致 → 判定 resume **未落地**，返回空触发 fresh 重建。
...
### 决策 3：增量回灌——快照水位（修正并发漏话）+ 排除自产

- **不用「完成时 MAX(messages.id)」做水位**（并发下会漏话：A 执行期间 B 写的消息 id 会被算进 A 的水位，导致 A 永远看不到那些话）。
- **改用「prompt-build 快照水位」`snapshot_msg_id`**：= 构建本次 prompt 那一刻该 conversation 的 `MAX(messages.id)`。水位**只推进到快照**，不含执行期间别人新写的。
- **增量 = `messages WHERE conversation_id=? AND id > snapshot_msg_id AND 非本 agent 自己产出`**（参数化 `id > ?`）：
  - 别人（含人工）在上一快照之后说的话 → 喂给本 Agent。
  - 本 Agent 自己的历史发言由 CLI session 记忆承载，**不重喂**。
  - A 执行期间 B 写的消息 id > A 的旧快照 → 下次触发 A 时被正确纳入增量，**不漏**。
...
| 情形 | 处置 |
|------|------|
| 首次执行(无 `agent_sessions` 行) | 开新 session（不带 resume）+ **全量回灌**；成功后存 session_id + snapshot_msg_id |
| resume 未落地（"no conversation found"、emitted id≠请求 id） | 清失效 session_id → 本次降级全量 + 新建 → 更新 session_id |
...
- **claude（改动小）**：现状 `-p ... stream-json` + prompt 走 stdin。resume = 追加命令行 flag `--resume <session_id>`（对标 `claude.go:605`）。session_id 从 stream-json 的 system/result 行提取。
- **codex（改动大，集成模式变更）**：现状 `codex exec --json -m - -`（一次性）。resume 需改为 **`codex app-server --listen stdio://` 长驻进程 + JSON-RPC**：
...
- **T3 增量取历史**：`runner.py` 执行前查 `agent_sessions`，决定「全量首建 / 增量 + resume」；增量 SQL 参数化 `id > ? AND author != 本agent`；快照水位 = prompt-build 时 `MAX(messages.id)`。
...
| 里程碑 | 交付 | 价值 | 验证 |
|--------|------|------|------|
| **S1** claude session 建立 | 建表 + 串行约束；抓 session_id（pin + COALESCE 覆盖）；首次全量 | session 被正确建立/存储/可查 | 探针:首次执行后 `agent_sessions` 有正确 session_id + snapshot_msg_id；同 (task,agent) 不并行起两条 |
| **S2** claude resume + 增量回灌 | `--resume` + 快照水位增量（排除自产）；resolveSessionID 落地判定 | **省 token + 上下文连贯 + 并发不漏话** | 探针:二次执行带 resume、prompt 只含增量、并发场景 B 在 A 跑期间的发言下次不漏；token 对比下降 |
