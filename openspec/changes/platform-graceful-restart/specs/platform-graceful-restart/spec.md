...
| `abandoned` | **无残留进程的非成功中止（Review 第十六轮 P1-A 扩义）**——正交字段 `abandon_stage(prelaunch\|running)` + `abandon_reason(lease_reclaim\|null_conversation_migration\|...)` 区分：`prelaunch/lease_reclaim`=claimed lease 回收、CLI 未起（**非定局**、execution 回 `queued`）;`running/null_conversation_migration`=running NULL 已确认完整进程树退出（**可作 final**、execution `recovery_blocked(null_conversation_migration)`、不回队，第十五轮 P1-3）。**定局性 SHALL 由 `final_attempt_id` 是否引用它决定、SHALL NOT 仅凭 `status=abandoned` 判非定局或回队性** | 是 |
| `orphaned` | **运行态进程未确认死亡的孤儿（Review 第八轮 P1-B）**——reclaim 发现残留 `running` attempt 但无法证明其 CLI 已停;与 `abandoned` 语义分开，因 `orphaned`=进程**未确认退出**（可能存活）、`abandoned`=**无残留进程**（未起 CLI 或已确认退出） | 是 |
...
**层间映射 SHALL 恒定**：execution 终态由**定局 attempt** 决定——定局 attempt=`succeeded` ⇒ execution=`done`;定局 attempt=`failed`（且恢复耗尽/不可重试，含 `failure_stage=prestart` 的不可重试准备失败）⇒ execution=`failed`;`orphaned`（未确认死亡的孤儿 attempt，第八轮 P1-B）驱动 execution=`recovery_blocked`(`process_not_confirmed_dead`) 且 `final_attempt_id` 指向它。**`abandoned` 的定局性按正交字段分（第十六轮 P1-A）**：`abandon_reason=lease_reclaim`（prelaunch）= **非定局**，execution 回 `queued`、不被 `final_attempt_id` 引用;`abandon_reason=null_conversation_migration`（running·已确认退出）= **定局**，驱动 execution=`recovery_blocked(null_conversation_migration)` 且被 `final_attempt_id` 引用（第十五轮 P1-3 running NULL 确认退出）。`superseded` 恒为非定局（随 execution 交棒中断）。**定局性 SHALL 由「是否被 `final_attempt_id` 引用」决定，SHALL NOT 仅凭 `status` 一刀切判 `abandoned` 非定局**。所有 execution 自然完成 CAS SHALL 明确写成 `SET status='done' WHERE status='running' AND worker_generation=? AND worker_instance_id=?`，SHALL NOT 写 `SET status='succeeded'`。
...
| `abandoned`·`null_conversation_migration`（running·已确认退出，第十六轮 P1-A） | 否（基础设施/迁移中止、非业务失败） | 标「迁移待处理」（关联 execution=`recovery_blocked(null_conversation_migration)`）;`final_attempt_id` 指向它 | 不触发自动流转;execution `recovery_blocked` 不回队、等人工迁移出口 |
...
| attempt | `running→abandoned`（NULL migration·已确认完整进程树退出，第十六轮 P1-A） | `running`（fencing 生效 AND 完整进程树确认退出后落 abandoned·running） | fencing 生效 **AND** 完整进程树确认退出 AND `blocked_reason=null_conversation_migration` 且 `process_cleanup_state=confirmed` | `abandon_stage=running`、`abandon_reason=null_conversation_migration` | 是（指该 abandoned attempt、driven execution=`recovery_blocked(null_conversation_migration)`） | `terminal(recovery_blocked)` |
| attempt | `running→orphaned`（protocol mismatch 且 gate 已释放/CLI 已起，第九轮 P0-C + 第十轮 source 对齐;或 NULL migration·进程树未确认退出，第十六轮 P1-C） | `running`（**gate 释放在 CAS 转 running 之后，故 source 必为 running 非 claimed**，见启动闸门 Requirement 步骤 2→3→4） | protocol mismatch AND launch gate 已释放（此时 attempt 已 running、进程未确认退出）;或 NULL migration 且 `process_cleanup_state=unconfirmed` | — | 是（指该 orphaned attempt） | `terminal(recovery_blocked)` |
...
- `recovery_budget_exhausted`：把人工给的**新预算值写入该 recovery chain 的预算记录 + 写审计**，child 带新 budget（SHALL NOT 原值重启后立刻再 blocked）;
...
**唯一冲突时 SHALL NOT 重复 apply 副作用（Review 第十二轮 P1-B）**：并发或重试触发第二次 `resume_blocked_execution()` 时，`UNIQUE(superseded_from)` 冲突后 SHALL **读回并返回已存在 child、直接返回 `idempotent_replay=true`**，SHALL NOT 再次加预算 / 再次清 session / 再次转移 pending / 再写一条审计——副作用只在首次成功建 child 的事务里发生一次。

**🔴 不同 token 并发输家 SHALL 持久化自己的 token→existing child 映射（第十五轮 P1-5A）**：`UNIQUE(superseded_from)` 冲突的**输家**（不同 `request_token`、child 已被赢家建）虽不再 insert child，但仍 SHALL 在**独立后续事务**里向 `recovery_requests` 插入自己的 `{parent_execution_id, request_token, payload_hash, child_execution_id=已存在 child, actor}` 映射行——否则该输家响应再次丢失后无法按其 token 稳定重放、后续「同 token 不同 payload」也无法可靠返回 409。故 SHALL 明确两个事务分支：
```text
首次获胜：reason 副作用 + budget CAS + child insert + recovery_requests(本 token→child) + events，同一事务
不同 token 命中已有 child（输家）：先整体回滚失败的 child-insert 事务（SHALL NOT 在失败事务里补偿写）
                              → 再开独立事务：不重复 reason 副作用/预算/event
                              → 仅插入 recovery_requests(本 token, payload_hash → existing child)
                              → 返回 idempotent_replay=true
```
两分支下 `recovery_requests` 的 `UNIQUE(parent_execution_id, request_token)` 保证每个 token 恰好一行映射;副作用（预算/session/pending/审计 event）只在赢家事务发生一次。
...
- **THEN** 父 execution 事件流保持封闭、**不出现第二个 terminal/superseded 事件、也不追加恢复事件**;统一的 `recovery_resumed{source:manual, recovery_parent_id, child_execution_id, blocked_reason}` 作为 **child 事件流的控制事件**（进统一 event_type、无 `manual_recovery` 第二名，第十三轮 P1-C），child `queued` 紧随;progress/Runtime/失败归因/`final_attempt_id` 层间映射不失真
...
- **THEN** `resume_blocked_execution()` 按 reason 校验不同前置——process 须确认全树退出、budget 须人工写新预算+审计、poisoned 须清 session 强制 full_replay、protocol 须有满足 floor 的 Worker;前置不满足 SHALL 拒绝建 child（不产生注定再次 blocked 的 child）;每种 reason 的 external pending 都不丢，kill 仍是唯一取消 pending 的路径

#### Scenario: reason 副作用写入与建 child 同一原子事务（第十二轮 P1-B）
- **WHEN** 对 `recovery_budget_exhausted` 恢复（写新预算+审计）、对 `poisoned_session` 恢复（清 poisoned pointer + 强制 full_replay），在插入 child 前后任一写入点注入故障
- **THEN** 预算写入/审计/清 session/转移 pending/插 child/写事件全在 `resume_blocked_execution()` 单次提交内，任一失败整体回滚，不出现「预算已加但 child 没建」「session 已清但恢复失败」半提交;成功则 child 带新预算（不原值立刻再 blocked）、poisoned 走 full_replay
...
- **请求体**：`{blocked_reason_ack, recovery_request}`——`recovery_request` 按 `blocked_reason` 携带对应前置证据（如 `process_not_confirmed_dead` 的「进程树已清理」确认、`recovery_budget_exhausted` 的新预算值 + 审计理由）;客户端 SHALL 携带 `manual_recovery_token`（请求幂等键）。
...
- **幂等**：同 `manual_recovery_token` 重试 SHALL 返回**同一** `child_execution_id` 且 `idempotent_replay=true`（第二层请求幂等）;父子基数由 `UNIQUE(superseded_from)` 硬约束保证（第一层）——同父不同 token 并发时，第二个 INSERT 冲突后 SHALL 读回并返回已存在 child、`idempotent_replay=true`，SHALL NOT 报 500。
...
**查询发现** `GET /executions/{parent_id}` SHALL 在响应中返回 `recovery_child_id`（无则 null）与 `recovery_resolution`（`unresolved`/`resumed`），使未持有 POST 响应的客户端能发现 child。

**`manual_recovery_token` 持久化** SHALL 用 `(parent_execution_id, manual_recovery_token)` 唯一索引 + 请求 payload hash + 生成的 `child_execution_id`，使「响应丢失后重试」返回同一 child、「同 token 不同 payload」返回 409、「不同 token 命中已有 child」返回已存在 child。它 SHALL NOT 承担父子基数（那是 `UNIQUE(superseded_from)`）。

**恢复控制事件 SHALL 统一为唯一的 `recovery_resumed{source: manual|reclaim, actor?, recovery_parent_id, child_execution_id, blocked_reason, request_token?}`（Review 第十三轮 P1-C，删除 `manual_recovery` 第二事件名）**——**自动 reclaim（后台交棒/接管）产生的恢复 `source=reclaim` 且不记为人工操作**，人工 POST 恢复 `source=manual` 带 `actor`;它是 **child 事件流的控制事件、进入统一 event_type 真相源**，SHALL NOT 追加到已封闭的父事件流、SHALL NOT「child SSE 或独立审计表」两种实现并存（统一 child SSE 控制事件，另需审计则作其落库派生）。**同事务写序 SHALL 固定为**：`insert child execution → recovery_resumed(child_execution_id) → child queued(child_execution_id) → commit`,使 `recovery_resumed` 与 child `queued` 的全局 id 均在 child 事件流内单调递增。**child 订阅回放语义 SHALL 明确**：调用端从 `POST /resume` 响应拿到 `child_execution_id` 后，订阅 child 流 SHALL 从 child 自身起点回放（`WHERE execution_id=:child ORDER BY id`），SHALL NOT 携带父的较大全局 `Last-Event-ID` 而跳过 child 首个 `recovery_resumed`/`queued` 事件;经父 superseded 跳转的续订走 SSE Requirement 既有的「携带当前全局 Last-Event-ID + child queued 全局 id 必然更大」协议，两种口径不并存。
...
#### Scenario: 同 token 重试与同父不同 token 并发都返回同一 child
- **WHEN** 恢复响应丢失后客户端用**同一** `manual_recovery_token` 重试；另有后台 reclaim 用**不同 token** 并发恢复同一父
- **THEN** 同 token 重试经 `(parent_execution_id, token)` 唯一索引返回同一 `child_execution_id`、`idempotent_replay=true`;不同 token 并发时 `UNIQUE(superseded_from)` 冲突后读回已存在 child 返回、`idempotent_replay=true`，两路径都 SHALL NOT 建第二个 child、SHALL NOT 报 500
...
`recovery_blocked` 是**永久终态**、父状态不因恢复而改变（见「终态无出边」约束），故「该 execution 是否还阻塞」SHALL 是一个**派生态**而非父 `run_queue.status` 的新值，所有消费者 SHALL 从 `UNIQUE(superseded_from)` 关系派生，SHALL NOT 依赖改写父 status（Review 第十二轮 P1-A）。派生定义：

```text
unresolved blocked   = 父 status='recovery_blocked' AND NOT EXISTS(child WHERE superseded_from=父)
resumed blocked      = 父 status='recovery_blocked' AND EXISTS(child WHERE superseded_from=父)
...
| 消费者 | 口径 |
|---|---|
| dead-letter / 人工待办 | 只列 `unresolved blocked`（有 child 即移出待办）;SHALL NOT 把 `resumed blocked` 仍列为待恢复 |
| Runtime/RunRow 人工「确认已清理后重试」按钮 | `unresolved blocked` 才可点;`resumed blocked` 后按钮禁用/改为「查看恢复 child」跳转 |
...
- **终态**（done/failed/killed/superseded/recovery_blocked）：`current_attempt_id = NULL`、`final_attempt_id = 定局 attempt`（成功=succeeded attempt;失败/被杀=最后一个 running/prestart attempt;superseded=触发交棒的 attempt;NULL migration confirmed=该 `abandoned` attempt;NULL migration unconfirmed=该 `orphaned` attempt）。
- **🔴 终态 `final_attempt_id = NULL` 的唯一例外（第十六轮 P1-B）**：`status=recovery_blocked AND blocked_reason=null_conversation_migration AND source_status=queued`（从未 claim、无任何 attempt）时 `final_attempt_id = NULL` 合法。**除此唯一例外，所有终态 SHALL `final_attempt_id` 非 NULL**——SHALL 用 DB CHECK 等价校验（应用层/迁移探针）拒绝其他终态 `final=NULL`;GET detail / Runtime / progress / terminal serializer SHALL 有该空指针分支（recovery_blocked 且 final=NULL 时展示「NULL 会话待迁移」、不报错/不误判）;expand/backfill SHALL NOT 为该例外制造虚假 attempt（queued NULL 隔离为该终态时不回填 final、不新建占位 attempt）。
...
- **THEN** 该终态 `final_attempt_id=NULL` 合法（唯一例外：`recovery_blocked AND null_conversation_migration AND source_status=queued`）;GET detail/Runtime/progress/terminal serializer 走空指针分支展示「NULL 会话待迁移」不报错;expand/backfill SHALL NOT 为它造虚假 attempt

#### Scenario: 非例外终态 final=NULL 被拒绝（第十六轮 P1-B）
- **WHEN** 构造一个 `done`/`failed`/非 null-migration 的 `recovery_blocked` execution 却 `final_attempt_id=NULL`
- **THEN** DB CHECK 等价校验（应用层/迁移探针）拒绝该状态;final pointer 探针同时覆盖「合法 queued-NULL 例外放行」与「其他终态 final=NULL 拒绝」两侧
