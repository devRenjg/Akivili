...
- **THEN** 事件流只出现 `manual_recovery` + child `queued`，父 execution **不出现第二个 terminal/superseded 事件**;SSE 客户端即使在父 terminal 后关闭父订阅，也能经 `manual_recovery` 事件/查询发现 child;progress/Runtime/失败归因/`final_attempt_id` 层间映射不失真
