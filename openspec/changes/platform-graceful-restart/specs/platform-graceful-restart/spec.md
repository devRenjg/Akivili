...
- **THEN** 每次 lease reclaim 回队 SHALL 在回收事务内以 `remaining>0 AND execution_attempt_retry_version=expected` CAS 扣减 `execution_attempt_retry_remaining`（与瞬时/prestart/protocol gate 共用同一预算、崩溃重放不重复扣）;扣至 0 后 SHALL NOT 再回队，execution 落终局 `failed`（计失败率、按 pending 建普通 successor）;**SHALL NOT 出现无限 `claimed→abandoned→queued` crash-loop**
...
6. **🔴 普通 successor 因果键固化（第十九轮 P1-3，否则无法判 failed 父是否已被 pending successor 承接）**：recovery lineage 有 `superseded_from`、migration lineage 有 `migration_from_execution_id`，但 external pending 与 history backlog 承接的**普通 successor** 此前无确定的 predecessor 键，导致「一个 failed 父是否已被 pending successor 续跑」无法判定（若父仍算独立叶子，后续 successor 即使 `done`、父仍因旧 failed 永久判失败;若无条件排除父，又会隐藏真正独立的失败 intent）。故固化统一持久列（写入 proposal 顶层 `run_queue` schema）：
   ...
   - **🔴 三键 lineage 硬约束 SHALL 与 recovery chain 同等级固化（第二十轮 P1-2，不能只写「三键互斥」而无 DB/事务约束，否则多 child/重复消费/漏消费/迁移引用错表）**：`continuation_from_execution_id` 与 `superseded_from`/`migration_from_execution_id` 三者 SHALL 共享同一套结构约束——
     ...
     - **一父不被三类 child 同时引用**：同一 parent execution SHALL NOT 同时被 recovery child（superseded_from）、migration successor（migration_from_execution_id）、continuation successor（continuation_from_execution_id）引用——三个 partial unique 各自保证「该键至多一子」，中心化校验再保证「一个父在三键并集下至多一个后继」（`NOT EXISTS(任一后继键=父)` 才允许建新后继）。
     ...
     - **长期可选统一模型**：若三键分列维护成本过高，可长期收敛为单一 `execution_edges(parent_execution_id, child_execution_id UNIQUE, edge_kind ∈ {recovery|migration|external_pending|history_backlog})` 边表（`parent_execution_id` 上按 edge_kind 分组唯一或全局唯一按需），把上述约束收敛到一张表——本轮 SHALL 至少固化三列模型的完整约束，边表作为可选演进方向记录。
...
#### Scenario: continuation 一父一子 + 三键互斥硬约束（第二十轮 P1-2）
...
- **THEN** `UNIQUE(continuation_from_execution_id) WHERE NOT NULL` 挡住一父两 continuation child;「至多一个前驱键非 NULL」CHECK 等价（中心事务+BEFORE 触发器）挡住多前驱键;「一父在三键并集下至多一后继」中心校验挡住父被 recovery+continuation 双引用;三者均在建 child 事务原子校验（非先查后插），失败整体回滚
...
- **🔴 两层预算严格分账（第十九轮 P1-1，SHALL NOT 混用）**：系统有两个语义不同的预算真相源，各扣各的、互不替代——
  - **`run_queue.execution_attempt_retry_remaining` + `execution_attempt_retry_version`（attempt 级重试预算，execution 维度，第十九轮 P1-1 + 第二十轮 P1-1 补完整可迁移 schema）**：**仅**在**同一 execution 内重试**（瞬时失败 retryable、prestart failure retryable、protocol gate 未释放回队、**claimed lease reclaim 回队**）落 attempt 终态并回 `queued` 时扣减，**与 attempt 终态 + owner retire（epoch+1）+ `retry_scheduled` 事件同一 `finish_execution()`/回收事务**。扣至 0 后该 execution SHALL NOT 再回队重试：瞬时/prestart/lease reclaim 类落终局 `failed`（计失败率、按 pending 建普通 successor），protocol gate 类**恰好一次**落 `recovery_blocked(protocol_incompatible)`。此预算**不建 recovery child**、不动 `recovery_budgets`。schema 与生命周期 SHALL 固定（第二十轮 P1-1，用户拍板方案 A）：
