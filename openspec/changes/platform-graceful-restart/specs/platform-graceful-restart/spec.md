...
API 的 SSE 端点 SHALL 把输出近实时推送给前端而非直连 CLI stdout。**🔴 续传游标 SHALL 用 per-execution 提交顺序安全序号 `(execution_id, event_seq)`，`event_seq` 由 execution 行锁在事务内原子分配（PostgreSQL 目标真相源，第二十一轮 P0-1）——SHALL NOT 用跨 execution 的全局自增 `id`/`BIGSERIAL`/`IDENTITY` 直接充当已提交水位，SHALL NOT 用 `run_logs.id`，SHALL NOT 用 `SELECT MAX(seq)+1` 自算 seq**。根因（第二十一轮 P0-1）：PostgreSQL sequence/identity 在 INSERT 时即分配、**不等事务提交**，故序号顺序 ≠ 提交可见顺序——若用全局 id 作游标，一个先拿到小 id 但后提交的写入，会被「已看到大 id 先提交行」的 reader 永久越过（reader 之后只查 `id > 大id`），导致 SSE 永久漏投控制事件、断线重连也补不回。改用 **execution 行锁分配的 per-execution `event_seq`**：统一事件表定义为 `execution_events(execution_id, event_seq, event_type, payload_json, created_at, PRIMARY KEY(execution_id, event_seq))`，同 execution 的所有 event writer SHALL 在事务内先锁该 execution 行（PG `SELECT next_event_seq FROM run_queue WHERE id=:eid FOR UPDATE`、SQLite 过渡期靠单写事务）取 `next_event_seq`、写 event、`next_event_seq+=1` 同事务提交——**同 execution 内不存在「未提交 seq 被后序 seq 越过」的窗口**（行锁串行化了同 execution 的 seq 分配与提交）。控制事件（queued/**run_claimed**/run_started/**retry_scheduled**/superseded/**recovery_resumed**/terminal）不写在 run_logs，用 log id 做游标会导致断线期间的控制事件（如 superseded 跳转）重连后漏投或错序;`event_seq` 在 execution 内严格递增无未提交空洞，续传 `WHERE execution_id=:eid AND event_seq > :last_seq ORDER BY event_seq` 提交顺序安全。**SQLite 过渡兼容**：SQLite 单写事务下 `AUTOINCREMENT` 全局 id 无 late-commit 越位问题、可作 event_seq 的过渡实现，但契约真相源是 `(execution_id, event_seq)` 语义、迁 PostgreSQL 时 SHALL 按行锁分配落地，SHALL NOT 把「SQLite 全局 id 恰好单调」写成永久契约（见 [platform-concurrency-scaling] PG-first 实施 DAG，第二十一轮 P1-5）。**`recovery_resumed` 是统一恢复控制事件（Review 第十三轮 P1-C，替代 `manual_recovery` 第二事件名——SHALL NOT 再保留 `manual_recovery` 作为独立 event_type）**：人工恢复与自动 reclaim 恢复**共用同一个** `recovery_resumed{source: manual|reclaim, recovery_reason, actor?, recovery_parent_id, child_execution_id, blocked_reason?, request_token?}` 事件，`source` 区分来源、`recovery_reason(handover|orphan_reclaim|manual_blocked_resume)` 区分承接语义（第十七轮 P2-B）;**reclaim 交棒承接 `superseded` 父时无 `blocked_reason`（该字段缺省/null），仅承接 `recovery_blocked` 父时 `blocked_reason` 必填**;它是 **child 事件流的控制事件、进入统一 event_type 真相源**（前端消费者、回放协议、事件事务探针一并覆盖），SHALL NOT「写 child SSE 流 或 写独立审计表」两种实现并存——统一走 child SSE 控制事件（若另需持久审计，SHALL 作为该控制事件的落库派生、非第二套 event_type）。`run_claimed`（领取、CLI 未起）与 `run_started`（CLI 启动）SHALL 是两个不同事件（第九轮缺陷3），SHALL NOT 让 `queued→claimed` 与 `claimed→running` 都发 `run_started`。**`retry_scheduled` 为 attempt 边界控制事件（Review 第七轮 P1-6）**：同一 execution 多 attempt 共用一条事件流，attempt#N failed 回队时 SHALL 与 attempt 状态转换**同事务**写 `retry_scheduled{attempt_id, attempt_no, failure_stage, failure_class, retryable, next_retry_at}`——否则前端只会先后看到两个 `run_started` 而无法解释中间的失败、退避与 attempt 编号变化;execution terminal event 只在 execution 真正落终态时出现。所有 SSE 事件（log 与全部控制事件）SHALL 先落 `execution_events` 再推，`id: <execution_id>:<event_seq>` 为唯一游标（per-execution 提交顺序安全序号，第二十一轮 P0-1，SHALL NOT 退回全局自增 id）;`run_logs` SHALL 加 `meta_json` 列承载 log 事件的结构化附加信息（channel/tool/tool_input/tool_output），但 **log event 的 `execution_events.payload_json` 只存引用 `{run_log_id, channel, tool?}`、SHALL NOT 双存 content/tool_output**（正文只在 run_logs + meta_json，单一真相源，第二十一轮 P1-6），log 事件可从 run_logs + meta 完整重建。**状态/数据写入与其对应事件写入 SHALL 同事务提交**（POST 消息+queued execution+queued event；queued→claimed+run_claimed event；claimed→running+run_started event；run_log+log event；终态+terminal event；superseded+recovery child+superseded event+child `recovery_resumed{source=reclaim}` event+child queued event），SHALL NOT 分离提交而出现「有日志无 event」「已终态无 terminal event」「有 recovery child 无 superseded 跳转」「有 recovery child 无 recovery_resumed」。CLI 输出写 `run_logs` 的**调用入口与日志语义 SHALL 保持不变**(Worker 线程 → `_log()`)，但因 run_log 行与其 log event SHALL 同事务提交，`_log()` 内部落库 SHALL 改为「同事务写 run_logs + execution_events」，SHALL NOT 表述为「整条路径零改动」（Review 第四轮 P1-4）。SSE 断连重连 SHALL 携带 `Last-Event-ID: <execution_id>:<event_seq>` 从该 execution 的该 seq 之后回放。**🔴 切换到 successor execution 时游标 SHALL 用 `(execution_id, event_seq)` 从 child 起点回放、SHALL NOT 依赖跨 execution 的全局提交序（第二十一轮 P0-1 修正第四轮 P0-5 的全局 id 口径）**——每个 execution 有自己行锁分配的 `event_seq`，父的 `superseded` event 带 `successor_execution_id`，客户端据此显式切到 child、从 child 的 `event_seq=0` 起回放。supersede 时事件插入顺序 SHALL 固定为「父 CAS 转 `superseded` 并清 active → 建 recovery child execution → 写父 execution 的 `superseded{successor_execution_id}` event（父 event_seq） → 写 child 的 `recovery_resumed{source: reclaim}` event（child event_seq=0） → 写 child 的 `queued` event（child event_seq=1） → 提交」（**第十五轮 P1-1：自动 supersede/reclaim 建的 recovery child SHALL 同人工恢复一样发统一 `recovery_resumed`，只是 `source=reclaim` 不带人工 actor;此写序为全 change 唯一真相源，与 `finish_execution()` 收尾写序、design 双写同事务组合、tasks 1.6/1.6a/1.6c 逐字一致**）;客户端收到父 `superseded{successor_execution_id}` 后 SHALL 显式改订阅 child、按 `WHERE execution_id=:child AND event_seq > -1 ORDER BY event_seq` 从 child 起点无损获得 child 的 recovery_resumed/queued 及后续事件，**父子事件序不需要跨 execution 可比**（父子各自 per-execution 单调即可，不再依赖「child event 全局 id 必然大于父 superseded id」这一 late-commit 下不成立的全局假设），SHALL NOT 因「切 execution」把游标重置或漏取 child 首事件。**`recovery_resumed` event 的 payload 字段 SHALL 用 `source`（manual|reclaim），SHALL NOT 用 `recovery_source`——`recovery_source` 仅为 HTTP 响应体字段名，SHALL NOT 混进 SSE event payload（第十五轮 P1-1）**。（若产品选择 child 订阅不带 Last-Event-ID、完整回放 child，则 SHALL 明确幂等去重规则，两种口径不并存。）**terminal/superseded 后 SHALL 封闭旧 execution/attempt 的事件流（Review 第五轮 P1-8）**：终态或 superseded 事件写入后，对旧 attempt 的任何 log/control event append SHALL 再做 `owner/generation/status` CAS 校验——CAS 失败（该 attempt 已非当前 owner 或 execution 已终态）即丢弃该 event 或写入隔离审计表，SHALL NOT 进入用户事件流。否则迟到的父日志可能在父终态后又取得更大的父 `event_seq`，虽不漏数据但前端会看到终态后的父输出、甚至被错误消费者当成有效结果（per-execution seq 下这是同一 execution 内终态后追加的问题，仍由封流 CAS 挡住，与游标是否全局无关）。

**🔴 `recovery_blocked` 父的人工恢复 SHALL NOT 向已封闭的父事件流追加任何事件（含 `recovery_resumed`），发现 child 走「POST 响应 + 查询」而非父流迟到事件（Review 第十二轮 P0-B，修第十一轮引入的封流冲突）**：`superseded` 是父的**最后一个**事件、且它带 `successor_execution_id`，故交棒/reclaim 路径的客户端能在父流内收到 superseded 后据 `successor_execution_id` 显式切到 child、从 child 的 `event_seq=0` 起回放（per-execution 游标，第二十一轮 P0-1，见下方 supersede scenario）;但 **`recovery_blocked` 是终态、父流在 `terminal(recovery_blocked)` 后已永久封闭**，人工恢复发生在父终态**之后**，此时 SHALL NOT 再向父流写恢复事件（会破坏「终态是父流最后一个事件」的封流不变量、且父终态后关闭订阅的客户端也收不到）。因此：① **统一的 `recovery_resumed{source: manual|reclaim, recovery_reason, actor?, recovery_parent_id, child_execution_id, blocked_reason?, request_token?}` SHALL 作为 recovery child 事件流的控制事件**（进入统一 event_type 真相源、绑 child 的 execution_id，见 SSE Requirement 的写序;`recovery_reason` 必填、`blocked_reason` 仅承接 recovery_blocked 父时必填，第十七轮 P2-B），SHALL NOT append 到父 execution 的事件流、SHALL NOT 保留 `manual_recovery` 第二事件名（Review 第十三轮 P1-C）;② 触发恢复的调用端 SHALL 从 **`POST /executions/{parent_id}/resume` 响应体**直接得到 `child_execution_id`，据此订阅 child 流（`GET /executions/{child_id}/events`），不依赖父流迟到事件;③ 未持有 POST 响应的其他客户端 SHALL 经 **`GET /executions/{parent_id}` 查询** 得到 `recovery_child_id`/`recovery_resolution` 发现 child。父流封闭不变量对所有终态（含 `recovery_blocked`）一致成立。
...
- **WHEN** API 进程重启或网络抖动导致 SSE 断开,前端携带 `Last-Event-ID: <execution_id>:<event_seq>` 重连
- **THEN** 服务端 `WHERE execution_id=? AND event_seq > ? ORDER BY event_seq` 回放,log 与控制事件统一有序、不重复已收、不遗漏（含断线期间发生的 superseded/terminal）
...
#### Scenario: 并发写事件 per-execution seq 行锁分配不冲突
- **WHEN** API、Worker、日志线程、终态线程同时向同一 execution 的 execution_events 写入
- **THEN** 各 writer 在事务内先锁该 execution 行取 `next_event_seq` 再 +1，seq 分配被行锁串行化，各拿到不同单调 `event_seq`，不冲突、不乱序、不撞主键 `(execution_id, event_seq)`

#### Scenario: PostgreSQL 提交顺序：迟提交小 seq 不被越过（第二十一轮 P0-1）
- **WHEN** 同一 execution 下 T1 先在事务内锁行分配 `event_seq=100` 但未提交、T2 后到却因行锁阻塞（不能先拿 101 抢先提交），reader 在 T1 提交前只能读到 ≤99
- **THEN** 因 `event_seq` 由 execution 行锁分配、同 execution 的分配与提交被行锁串行化，reader 的 `event_seq > last_seq` 游标不会在 T1 提交前跨过 100；T1 提交后 seq=100 正常可见，**SHALL NOT 出现「小 seq 迟提交被 reader 永久越过」**（对比：全局 identity 在 INSERT 时分配不等提交，T2 可拿 101 先提交致 reader 越过 100，故 SHALL NOT 用全局 id 作游标）

#### Scenario: supersede 切 successor 用 per-execution seq 无损续订
- **WHEN** 父 execution 断线期间发生 supersede，同事务按「父 superseded event（父 seq） → child recovery_resumed(source=reclaim) event（child seq=0） → child queued event（child seq=1） → 提交」落库，客户端重连后收到父 `superseded{successor_execution_id}` 显式切订阅 child
- **THEN** 客户端按 `WHERE execution_id=child AND event_seq > -1 ORDER BY event_seq` 从 child 起点取到 child 的 recovery_resumed/queued 及后续事件，child recovery_resumed/queued/log/terminal 各恰好一次、child 内 seq 单调，**不依赖父子事件跨 execution 全局可比**、不因切 execution 漏取 child 首事件
...
### Requirement: execution_events retention、cursor expiry 与 log payload 单一真相源（第二十一轮 P1-6）

`execution_events` 是热表且随日志量增长，其 retention、游标过期与 log 内容的真相源 SHALL 闭环定义，SHALL NOT 只写一句「事件清理后过旧 Last-Event-ID 回放可得部分 + 提示当前状态」而留空生命周期。

- **🔴 log event payload = 只存引用，SHALL NOT 双份存储内容**：`log` 类 event 的 payload SHALL 固定为 `{run_log_id}` + 必需路由字段（`channel`、可选 `tool` 名等用于前端分流的最小字段），**内容（content/tool_input/tool_output）SHALL 只落 `run_logs`（+ `run_logs.meta_json`）**，SSE 读取时按 `run_log_id` 联表/批量加载。SHALL NOT 把 content/tool_output 同时写进 `execution_events.payload_json`（否则大工具输出双份存储、事件热表近似翻倍增长）。若为性能确需在 event 内缓存内容，SHALL 明确标注它**只是有界 cache**（有大小上限、可随时按 `run_log_id` 重建），非第二份真相源。
- **payload 大小上限与截断**：`execution_events.payload_json` SHALL 有大小上限;`tool_output` 等大字段 SHALL 只存 `run_log_id` 引用或截断摘要 + 「完整内容见 run_logs」标记，SHALL NOT 无界写入。控制事件 payload 字段固定、天然有界。
- **retention 策略 SHALL 明确**：SHALL 定义 `execution_events` 的保留时长/容量、按 execution 还是全局清理;`run_logs` 与 `execution_events` SHALL 有**一致的协调删除/归档顺序**——SHALL NOT 先删 `run_logs` 使 retained log event 的 `run_log_id` 悬空无法回放（引用完整性），SHALL NOT 只删一方造成「有 event 无 log」或「有 log 无 event」。删除 SHALL 按「先归档/删 event 再删其引用的 run_logs」或整组归档，保证任一时刻 retained event 的引用可解析。
- **cursor expiry 结构化状态**：SHALL 维护 `min_retained_event_seq`（每 execution 最早保留的 seq），客户端 `Last-Event-ID` 早于它时 SHALL 返回结构化 `cursor_expired` 状态（HTTP/SSE 层），SHALL NOT 静默漏投或当作从头。
- **reset snapshot 内容**：`cursor_expired` 后的 reset snapshot SHALL 明确包含哪些状态——当前 execution/attempt status、terminal 与否、最新 successor 指针、以及从 `min_retained_event_seq` 起可回放的事件，使前端能重建当前视图而非仅「部分事件 + 提示」。

#### Scenario: log event 只存引用不双份存储（第二十一轮 P1-6）
- **WHEN** CLI 产出大 `tool_output`，Worker 同事务写 `run_logs`（含 meta_json）+ `log` execution_event
- **THEN** `log` event.payload 只含 `{run_log_id, channel, tool?}`，大内容仅在 `run_logs`;SSE 读取按 `run_log_id` 联表加载内容，`execution_events` 不因大工具输出翻倍增长

#### Scenario: 游标过期返回结构化 cursor_expired + reset snapshot（第二十一轮 P1-6）
- **WHEN** 客户端带 `Last-Event-ID: <execution_id>:<event_seq>` 重连，但该 seq 早于该 execution 的 `min_retained_event_seq`（事件已按 retention 清理）
- **THEN** 服务端返回结构化 `cursor_expired` 状态 + reset snapshot（当前 execution/attempt status、terminal 与否、successor 指针、从 min_retained_event_seq 起可回放事件），SHALL NOT 静默漏投或从 0 重放已删区间

#### Scenario: run_logs 与 execution_events 协调删除不悬空（第二十一轮 P1-6）
- **WHEN** retention 清理触发
- **THEN** 清理 SHALL 保证任一时刻 retained log event 的 `run_log_id` 仍可解析——SHALL NOT 先删 `run_logs` 留下悬空引用的 event，按「event 与其引用 run_logs 整组归档/删除」协调顺序
...
- `recovery_budget_exhausted`：按**唯一 `grant_delta > 0`** 语义给该 recovery chain 补预算（`budget_limit += grant_delta`、`budget_remaining += grant_delta`、`version += 1` + 写审计 `last_reset_by/at/reason`，见下方「人工补预算」的 grant_delta 定义），child 带补足后预算（SHALL NOT 原值重启后立刻再 blocked）;SHALL NOT 用「写新预算值/覆盖 remaining」旧口径（绝对覆盖须走独立 admin override API）;**🔴 grant 与本次 child debit 的最终值 SHALL 在同一 `resume_blocked_execution()` 事务内确定性定死（第二十一轮 P1-4，消除「child 后 remaining=0」vs「remaining=1 还能再建一个」两种合法实现）——补预算(`+grant_delta`) 与建当前 recovery child 的消费(`-1`) 同时体现：`final budget_remaining = old_remaining + grant_delta − 1`、`final budget_limit = old_limit + grant_delta`、`final version = old_version + 2`（补预算 +1、建 child +1 各推进一次 version；实现可用合并 CAS 使 version 只推进一次，但最终 remaining/limit 算术 SHALL 与此等价）。** 例 `old_remaining=0, grant_delta=1` → 补后 remaining=1、建本 child 消费 −1 → `final remaining=0`（本 child 已建、不能再建第二个），SHALL NOT 出现「建 child 后 remaining 仍=1 还能再建」;探针 `grant_delta_child_debit_arithmetic_probe` 断言此终值。
...
**同事务内写入顺序 SHALL 固定为**：先把父 execution 转出 active 终态（转 `superseded`/终态并清 active 谓词）→ 再插入同 `(conversation, agent)` 的 successor（`queued`）→ 再按各 execution 的 per-execution `event_seq` 递增写事件（父 execution 的 terminal/superseded event 用父 event_seq → **supersede 路径追加 child 的 `recovery_resumed{source=reclaim}` event 用 child event_seq=0（第十五轮 P1-1，与 SSE Requirement 自动写序逐字一致）** → 子 queued event 用 child event_seq=1）→ 单次提交。SHALL NOT 先插 successor 再转父终态（active partial unique index 会因「父仍 active + 新 successor 同 conversation active」瞬时并存而拒绝插入）。
...
- **THEN** 事务内固定顺序为「父转出 active 终态 → 插同 conversation successor(queued) → 按各 execution 的 per-execution `event_seq` 递增写事件 → 单次提交」，active partial unique index 不因「父仍 active + 新 successor」瞬时并存而拒绝插入;SHALL NOT 先插 successor 再转父终态
...
- **THEN** attempt 落 `abandoned`，execution 回 `queued` 等兼容 Worker 重领、owner 退休、pending 保留;**回队走唯一公式（第二十一轮 P1-1）同事务扣减 `execution_attempt_retry_remaining`（失败时 old remaining>0→减 1+回队;old remaining=0→定局，非 `recovery_budgets.budget_remaining`——后者只在建 recovery child 时扣）**;因 CLI 从未起、无双执行风险，回队安全;反复不兼容直到某次失败发现 `remaining=0` 才**恰好一次**落 `recovery_blocked`(`blocked_reason=protocol_incompatible`)、`final_attempt_id` 指向末次 abandoned attempt，SHALL NOT 因两预算混用而无限 `abandoned→queued→claimed→abandoned`
...
- **THEN** 前 N-1 次每次 attempt 落 `abandoned`、execution 回 `queued`、`execution_attempt_retry_remaining` 单调递减且**只在成功提交的 `finish_execution()` 事务内扣减一次**（CAS `remaining>0 AND execution_attempt_retry_version=expected`、`version` 同步 +1，重放/并发不重复扣）;`remaining` 从 1 扣成 0 的那次仍完成本次回队（该 attempt 仍执行），**下一次**失败发现 `remaining=0` 才**恰好一次**落 `recovery_blocked(protocol_incompatible)`、`final_attempt_id` 指向末次 abandoned attempt，**全程 `recovery_budgets.budget_remaining` 不变**（未建任何 recovery child）;人工恢复才走 `resume_blocked_execution()` 建 child 并扣 `recovery_budgets`

#### Scenario: claimed lease reclaim 反复回队消费同一 retry 预算不无限（第二十轮 P1-1）
- **WHEN** 一个 execution 被 Worker claim 后 Worker 反复崩溃/心跳失效，`claimed lease` 反复超时回收（`claimed→abandoned→同 execution queued`），且回收事务在崩溃/并发下重放
- **THEN** 每次 lease reclaim 回队 SHALL 走**与其余三类来源完全相同的唯一公式**（第二十一轮 P1-1）：在回收事务内以 `remaining>0 AND execution_attempt_retry_version=expected` CAS——**失败时 old remaining>0 → 减 1 + 回 queued（该次 attempt 仍执行）;失败时 old remaining=0 → 当前 attempt 定局、不再回 queued**。故 `remaining` 从 1 扣成 0 的那次**仍完成本次回队**（与瞬时/prestart/protocol gate 逐字一致，SHALL NOT 在扣到 0 的当次拒绝回队），**下一次**失败发现 `remaining=0` 才落终局 `failed`（计失败率、按 pending 建普通 successor）;**SHALL NOT 出现无限 `claimed→abandoned→queued` crash-loop**（有界预算保证）
...
- **THEN** 走独立事务 `resume_blocked_execution()`，在一次提交内建恰好一个 recovery child（`queued`，`superseded_from=父`，有安全 session→`session_resume`/否则 `full_replay`）+ 写统一的 `recovery_resumed{source:manual}`/子 `queued` 事件（child 流、按 child 的 per-execution `event_seq` 递增、无 `manual_recovery` 第二名）;**父 execution 保持 `recovery_blocked` 永久不变、父 `final_attempt_id` 仍指向 orphaned attempt、SHALL NOT 发第二个 terminal/superseded 事件、不重复 retire owner、不推进 committed**;任一步失败整体回滚;复用 `superseded_from` 因果键，SHALL NOT 改父状态、SHALL NOT 新增 `recovered_from`、SHALL NOT 复用 `finish_execution()`
...
**`manual_recovery_token` 持久化** SHALL 用 `recovery_requests` 表的 `(parent_execution_id, request_token)` 唯一索引 + `canonical_payload_hash` + `outcome(winner|idempotent_replay|rejected_already_resolved)` + `resolved_child_id`（第十七轮 P1-D + 第十九轮 P1-2），使「响应丢失后重试」返回同一 child、「同 token 不同 payload」返回 409、「**不同 token·canonical 一致**」返回已存在 child（`idempotent_replay=true`）、「**不同 token·canonical 不同**」返回 409 `already_resolved`（SHALL NOT 静默映射不同意图到赢家 child）。它 SHALL NOT 承担父子基数（那是 `UNIQUE(superseded_from)` + `recovery_operations.child_execution_id UNIQUE`）。

**恢复控制事件 SHALL 统一为唯一的 `recovery_resumed{source: manual|reclaim, recovery_reason, actor?, recovery_parent_id, child_execution_id, blocked_reason?, request_token?}`（Review 第十三轮 P1-C + 第十七轮 P2-B，删除 `manual_recovery` 第二事件名）**——**自动 reclaim（后台交棒/接管）产生的恢复 `source=reclaim` 且不记为人工操作**，人工 POST 恢复 `source=manual` 带 `actor`;**`recovery_reason` SHALL 为必填枚举 `handover|orphan_reclaim|manual_blocked_resume`（第十七轮 P2-B）**——`source=manual` 恒 `manual_blocked_resume`（父必为 `recovery_blocked`、`blocked_reason` 必填）;`source=reclaim` 为 `handover`（正常交棒，父 `superseded`）或 `orphan_reclaim`（孤儿接管），**父为 `superseded` 时无 `blocked_reason`、该字段 SHALL 缺省/为 null，消费者 SHALL NOT 假定 `recovery_resumed` 恒带 `blocked_reason`**（reclaim 交棒父没有阻塞原因）;仅当承接的父处于 `recovery_blocked`（manual_blocked_resume，或 orphan_reclaim 命中 recovery_blocked 孤儿）时 `blocked_reason` 才必填。它是 **child 事件流的控制事件、进入统一 event_type 真相源**，SHALL NOT 追加到已封闭的父事件流、SHALL NOT「child SSE 或独立审计表」两种实现并存（统一 child SSE 控制事件，另需审计则作其落库派生）。**同事务写序 SHALL 固定为**：`insert child execution → recovery_resumed(child_execution_id, event_seq=0) → child queued(child_execution_id, event_seq=1) → commit`,使 `recovery_resumed` 与 child `queued` 在 child 自身的 per-execution `event_seq` 内单调递增。**child 订阅回放语义 SHALL 统一（第二十一轮 P0-1 收敛）**：无论是经 `POST /resume` 响应/查询发现 child（人工恢复），还是经父 `superseded{successor_execution_id}` 跳转发现 child（自动交棒/reclaim），客户端**都 SHALL 切到 child 的 `execution_id`、从 child 自身的 `event_seq` 起点回放**（`WHERE execution_id=:child AND event_seq > :last_child_seq ORDER BY event_seq`，首次订阅 `last_child_seq=-1`），从而不漏 child 首个 `recovery_resumed`/`queued` 事件。**旧的「两种口径」分岔（人工=不带父游标 / 自动=携带全局 Last-Event-ID 且 child 全局 id 必然更大）是全局游标的产物，per-execution `event_seq` 下二者归一为「切 execution + 从 child event_seq 起点回放」同一协议，SHALL NOT 再保留跨 execution 全局 id 可比假设**。
...
- **WHEN** 恢复响应丢失后客户端用**同一** `manual_recovery_token` 重试；另有第二个人工操作者用**不同 token 但相同 canonical payload** 并发恢复同一父（后台 reclaim 无 HTTP token，走 `recovery_operations` 的 `operation_source=reclaim` 分支、不进 `recovery_requests`）
- **THEN** 同 token 重试经 `recovery_requests(parent_execution_id, request_token)` 唯一索引返回同一 `child_execution_id`、`idempotent_replay=true`;不同 token·canonical 一致时 `UNIQUE(superseded_from)` 冲突后比对 `canonical_payload_hash`==赢家 `recovery_operations.canonical_payload_hash` 通过、读回已存在 child 返回、`idempotent_replay=true`，两路径都 SHALL NOT 建第二个 child、SHALL NOT 报 500
...
- **THEN** 输家 `UNIQUE(superseded_from)` 冲突后比对 `canonical_payload_hash` != 赢家 `recovery_operations.canonical_payload_hash`，返回 **409 `already_resolved`**（附赢家 `child_execution_id` + 其 `canonical_payload_hash`）、独立事务只写 `recovery_requests(token, canonical_payload_hash, outcome='rejected_already_resolved')` 审计;SHALL NOT 把输家的不同意图静默映射到赢家 child（否则输家误以为自己的 `grant_delta`/模式生效）、SHALL NOT 建第二个 child、SHALL NOT 报 500

#### Scenario: 自动 reclaim 获胜写 recovery_operations 不进 recovery_requests（第十九轮 P1-2 / 第二十轮 P1-3 改名）
- **WHEN** 后台 reclaim（无 HTTP token/actor）承接一个父 execution 建 recovery child
- **THEN** SHALL 在 reclaim 事务内写 `recovery_operations{parent, child, operation_source=reclaim, canonical_payload_hash(规范化 reclaim operation)}`（第二十轮 P1-3：本表只承载 recovery canonical 幂等、无 resolution_kind 列），**SHALL NOT 试图写 `recovery_requests`**（无 request_token/actor）;此后人工输家比对赢家意图时读 `recovery_operations.canonical_payload_hash`——reclaim 赢家也有可比对的 canonical 指纹，不会因「赢家是自动分支」而无从比对
...
#### Scenario: recovery child 事件写序与 per-execution event_seq 单调（第十三轮 P1-C + 第二十一轮 P0-1）
- **WHEN** `resume_blocked_execution()`/reclaim 建 child
- **THEN** 同事务写序固定为 `insert child execution → recovery_resumed(child_execution_id, event_seq=0) → child queued(child_execution_id, event_seq=1) → commit`;任一点故障整体回滚（无「有 child 无 recovery_resumed」「有事件无 child」半提交）;child 流内 `recovery_resumed` 与 `queued` 在 child 自身 per-execution `event_seq` 内单调递增，SHALL NOT 依赖跨 execution 全局 id 单调

#### Scenario: child 订阅从自身起点回放不漏首事件（第十三轮 P1-C + 第二十一轮 P0-1 口径归一）
- **WHEN** 调用端经 `POST /resume` 响应/查询（人工恢复）或经父 `superseded{successor_execution_id}` 跳转（自动交棒/reclaim）发现 `child_execution_id` 后订阅 child 流
- **THEN** 两种发现路径**统一**从 child 自身 event_seq 起点回放（`WHERE execution_id=:child AND event_seq > :last_child_seq ORDER BY event_seq`，首次 `last_child_seq=-1`），都能看到 `recovery_resumed`/`queued` 首事件;SHALL NOT 携带父的较大全局 `Last-Event-ID` 或依赖「child 全局 id 必然大于父」跨 execution 假设;**per-execution `event_seq` 下人工/自动两种口径归一为「切 execution + 从 child event_seq 起点回放」同一协议、不再分岔**
...
effective chain state= 沿 superseded_from 链一路走到叶子 execution，取叶子的 status
                       （叶子可能是 running/done/failed/又一个 recovery_blocked…；
                        migration successor 不在 superseded_from 链上、由全因果叶子优先级聚合承接）
```
...
**🔴 effective chain 只表 recovery lineage，普通 successor 另由全因果叶子优先级聚合判定（用户拍板，第十三轮 P1-D；第二十轮 P1-2 归一措辞——非 task 级单值 active 聚合）**：`effective chain state` 仅沿 `superseded_from` 恢复链聚合——它只覆盖 supersede/交棒/人工恢复产生的 recovery child。**recovery child 后续因 external pending 或 history backlog 创建的普通 successor 是新的 execution、不挂在 `superseded_from` 恢复链上**（它们用普通 successor 因果键 `continuation_from_execution_id` + `continuation_kind`，第十九轮 P1-3 归一），故 SHALL NOT 期望「沿 `superseded_from` 走叶子」覆盖普通 successor。任务整体完成度 SHALL 由**下方「task 级完成度聚合的确定性口径」Requirement 的全因果叶子优先级聚合**判定，recovery lineage 只回答「这条被中断的执行线是否已续上、续到哪」。**SHALL NOT 用「该 task 是否还有 active execution / 最新 execution 终态」这类单值口径**（第十九轮 P1-3：该旧顶层措辞与全叶子聚合冲突——多条并行 lineage 下「最新终态」会掩盖较早仍 unresolved 的 blocker）;SHALL NOT 默认「只沿 `superseded_from` 就覆盖所有后续执行」。
...
**🔴 resolved 判定唯一权威 = 因果键纯派生，SHALL NOT 三套真相源（第十九轮 P1-5 + 第二十轮 P1-3 收口：`recovery_operations`/因果键/cache 三者不得并称真相源）**：**「该父是否已恢复（resolved）」的唯一权威恒为因果键 `EXISTS(...)` 纯派生**——recovery 类 = `EXISTS(child WHERE superseded_from=父)`;migration 类 = `EXISTS(successor WHERE migration_from_execution_id=父)`（第十七轮 P1-A）;SHALL NOT 只按 `superseded_from` 判定（否则 migration 已建 successor 但仍显未 resolved、NULL migration 父永久卡待办）。**`recovery_operations` 表 SHALL NOT 作为 resolved 判定来源**——它只承载 recovery 分支建 child 的 canonical/幂等（第二十轮 P1-3 改名收窄），是因果键的一致副本、不是第二套判定。**默认口径 = 纯派生、不建 cache**（所有消费者直接跑上述 `EXISTS` 派生查询）。**若为性能必须缓存，SHALL 采用完整 schema 且明确定义为因果键的派生副本、SHALL NOT 用可漂移的松散冗余列、SHALL NOT 升为独立真相源**：增设 `run_queue.resolution_successor_id`（NULLABLE，指向承接 child/successor）+ `resolved_at`/`resolved_by`，`resolution_successor_id` 加 `UNIQUE ... WHERE resolution_successor_id IS NOT NULL`，SHALL 与承接 child/successor insert **同一事务写入**（recovery 类同 `resume_blocked_execution()`、migration 类同 migration 事务），SHALL 提供回填（对存量已 resumed 的父按因果键回填 cache）+ 一致性探针（cache 与派生 `EXISTS` 结果恒等，漂移即报警且以因果键为准）。是否启用 cache SHALL 全局统一选定，SHALL NOT 一处纯派生、另一处必写 cache。
...
- **THEN** 沿 `superseded_from` 的 `effective chain state` 停在该 recovery child（recovery lineage 已到叶子），任务整体完成度由**全因果叶子优先级聚合**判定（普通 successor 作为新的叶子 execution 参与）;SHALL NOT 因 progress 只沿 `superseded_from` 而停在旧 recovery child、漏看普通 successor
...
6. **🔴 普通 successor 因果键固化（第十九轮 P1-3，否则无法判 failed 父是否已被 pending successor 承接）**：recovery lineage 有 `superseded_from`、migration lineage 有 `migration_from_execution_id`，但 external pending 与 history backlog 承接的**普通 successor** 此前无确定的 predecessor 键，导致「一个 failed 父是否已被 pending successor 续跑」无法判定（若父仍算独立叶子，后续 successor 即使 `done`、父仍因旧 failed 永久判失败;若无条件排除父，又会隐藏真正独立的失败 intent）。故固化统一持久列（写入 proposal 顶层 `run_queue` schema）：
   ...
   - **🔴 三键 lineage 硬约束 SHALL 与 recovery chain 同等级固化（第二十轮 P1-2，不能只写「三键互斥」而无 DB/事务约束，否则多 child/重复消费/漏消费/迁移引用错表）**：`continuation_from_execution_id` 与 `superseded_from`/`migration_from_execution_id` 三者 SHALL 共享同一套结构约束——
     - **一父至多一子**：`CREATE UNIQUE INDEX ... ON run_queue(continuation_from_execution_id) WHERE continuation_from_execution_id IS NOT NULL`（镜像 `UNIQUE(superseded_from)`/`UNIQUE(migration_from_execution_id)`）。
     - **每 child 至多一个前驱键非 NULL**：`CHECK`（等价）——`superseded_from`/`migration_from_execution_id`/`continuation_from_execution_id` 三列至多一个非 NULL（SQLite 无法 ALTER 加表级 CHECK → 中心化 child-insert 事务 + `BEFORE INSERT/UPDATE` 触发器 + 迁移/CI 探针做等价校验）。
     - **🔴 一父不被三类 child 同时引用 = `execution_edges` 统一边表硬约束（第二十一轮 P0-2，从「长期可选」提升为近期硬约束）**：同一 parent execution SHALL NOT 同时被 recovery child（superseded_from）、migration successor（migration_from_execution_id）、continuation successor（continuation_from_execution_id）引用。**SHALL NOT 仅靠「三个分列 partial unique + 中心化 `NOT EXISTS(任一后继键=父)`」**——PostgreSQL `READ COMMITTED` 下两个事务分别插不同列（如 T1 recovery superseded_from=P、T2 continuation continuation_from=P），彼此看不到对方未提交行、各命中不同 partial unique 均可提交，`NOT EXISTS` 挡不住，最终一父两后继（双执行、pending 转移与副作用归属失真）。故 SHALL 建 **`execution_edges(parent_execution_id, child_execution_id, edge_kind ∈ recovery|migration|external_pending|history_backlog)` + `UNIQUE(parent_execution_id)` + `UNIQUE(child_execution_id)`**：三类 successor helper 在**同一事务先 `INSERT execution_edges`（命中 `UNIQUE(parent_execution_id)` 即冲突失败=已被承接）再建 child**——单一 `UNIQUE(parent_execution_id)` 由数据库保证跨键并集唯一（并发第二个 INSERT 必冲突失败），无论后继属哪一类。三个 predecessor 列（superseded_from/migration_from_execution_id/continuation_from_execution_id）保留作查询/审计/kind 判别，但「一父至多一后继」以 `execution_edges` 为准。（SQLite 过渡期同样建此边表 + UNIQUE，单写事务下天然串行;PG 下由 `UNIQUE(parent_execution_id)` 保证，见 [platform-concurrency-scaling] PG-first DAG。）
     - **scope 一致**：continuation parent/child 的 `task_id/conversation_id/agent_slug` SHALL 一致（普通 successor 承接同 task 同 conversation 同 agent 的执行线;它开自己的新 recovery chain，故 `recovery_chain_id` 不要求等父）。
     - **创建后不可变 + 删除守卫**：`continuation_from_execution_id`/`continuation_kind` 创建后永久不可变（与 recovery lineage 字段同）;删除仍被 continuation child 引用的前驱 SHALL 被拒绝或整组归档;禁自引用 `child.id != continuation_from_execution_id`。
     - **kind ↔ parent 收尾路径合法匹配**：`continuation_kind=external_pending` 的 child SHALL 由「父终局 failure/success 有 external pending intent」路径创建;`continuation_kind=history_backlog` 的 child SHALL 由「父成功且 `batch_scan_end<planned_through`」backlog 续批路径创建;SHALL NOT 用 continuation 键承接 recovery/migration 语义（那两类各有专键）。
     - **中心化 helper + integrity scan**：所有 continuation successor 创建经统一 helper（与 recovery child、migration successor 的 helper 同级），SHALL NOT 散落多处各写校验;周期/迁移后 integrity probe 扫描 continuation 的 dangling parent/scope drift/多前驱键/被多类 child 引用的父。
     - **统一边表为并集唯一真相源（第二十一轮 P0-2 定稿）**：`execution_edges` 已从「长期可选」提升为**近期硬约束**（见上一条），「一父至多一后继」跨键并集唯一以 `UNIQUE(execution_edges.parent_execution_id)` 为数据库级真相源。三个 predecessor 列继续保留作查询/审计/kind 判别与全叶子聚合；分列 partial unique 仍建（挡同列一父两子），但**跨键并集唯一不再依赖中心化 `NOT EXISTS`**（PG 并发下不成立），一律以边表 UNIQUE 为准。
   - **与既有 `run_queue.source_run_id` 的关系（第二十轮 P1-2 纠正字段位置）**：现网 `source_run_id` 是 **`run_queue` 表的列**（`database.py`，注释「触发本次入队的上游 `task_runs.id`」）——即它挂在 execution 行上、但**值引用一个 task_run(attempt) id**、语义是「哪个 attempt 触发了本次入队」的旧因果溯源。`continuation_from_execution_id` 是新的 **execution→execution 因果键**（值引用前驱 execution 的 `run_queue.id`），语义是「本普通 successor 承接哪个前驱 execution」。二者引用维度不同（一个指 task_run、一个指 execution）不互相替代;`source_run_id` SHALL 保留其现有 attempt 溯源语义、SHALL NOT 被改写承担 execution 级 continuation 判定（避免第二套口径）。
...
#### Scenario: continuation 一父一子 + 三键互斥硬约束（第二十轮 P1-2 + 第二十一轮 P0-2）
- **WHEN** 并发或重放对同一前驱 execution 建两个 continuation successor、或对某 child 同时写 `superseded_from` 与 `continuation_from_execution_id`、或对已被 recovery child 引用的父再建 continuation successor
- **THEN** 建 child 事务先 `INSERT execution_edges(parent_execution_id=父,...)`，`UNIQUE(execution_edges.parent_execution_id)` 挡住一父两后继（无论同键或跨键）;`UNIQUE(continuation_from_execution_id) WHERE NOT NULL` 分列再挡同键一父两子;「至多一个前驱键非 NULL」CHECK 等价挡住某 child 多前驱键;均在同一事务原子校验（非先查后插），失败整体回滚

#### Scenario: 跨键 successor 并发只建一个后继（第二十一轮 P0-2，PG READ COMMITTED）
- **WHEN** 三个事务在 PostgreSQL `READ COMMITTED` 下同时对同一父 execution P 分别建 recovery child（superseded_from=P）、migration successor（migration_from_execution_id=P）、continuation successor（continuation_from_execution_id=P），彼此看不到对方未提交行
- **THEN** 三者均在同一事务先 `INSERT execution_edges(parent_execution_id=P, edge_kind=各自类)`——`UNIQUE(parent_execution_id)` 使**恰好一个事务提交成功**，另两个 INSERT 冲突失败、整体回滚并得到结构化「已被承接（附赢家 child_execution_id/edge_kind）」结果，SHALL NOT 产生第二个 child 或重复副作用;**SHALL NOT 依赖三分列 partial unique + `NOT EXISTS`**（那在 READ COMMITTED 下三事务各命中不同分列 unique 均可提交、`NOT EXISTS` 看不到未提交行，会建三个后继）

#### Scenario: continuation kind 与父收尾路径匹配、字段不可变（第二十轮 P1-2）
- **WHEN** 尝试用 `continuation_kind=history_backlog` 承接一个「父 failed 有 external pending」的 external_pending 场景、或事后修改已建 continuation child 的 `continuation_from_execution_id`/`continuation_kind`、或删除仍被 continuation child 引用的前驱
- **THEN** kind↔父收尾路径不匹配被中心 helper 拒绝（external_pending 只由 pending 路径建、history_backlog 只由 backlog 续批路径建）;字段创建后不可变、update 被守卫+触发器拒绝;删除被引用前驱被守卫拒绝或整组归档;禁自引用 `child.id != continuation_from_execution_id`
...
- **🔴 恢复真相源拆为两表（第十九轮 P1-2，纠正 `canonical_request_id` 名为 ID 实为 hash、且自动 reclaim 无 HTTP token 无法进人工请求表）**：
  - **`recovery_operations` 表（第二十轮 P1-3 从 `recovery_resolutions` 改名 + 收窄职责：**仅承载 recovery canonical operation 与幂等**，manual 与 reclaim 共用，**SHALL NOT 再称「resolved 真相源」**）**：`{parent_execution_id PK, child_execution_id UNIQUE, operation_source(manual|reclaim), canonical_payload_hash, created_at}`（**删去 `resolution_kind` 列**——migration 不写本表、其承接身份由 `migration_from_execution_id`+`null_migration_requests` 表达，本表只管 recovery 分支的 canonical 幂等）。它记录「这个 recovery 父被谁、以什么规范意图、承接到哪个 recovery child」——**recovery 分支的自动 reclaim 与人工 resume 都 SHALL 写这里**（reclaim 无 request_token/actor，故不进 `recovery_requests`）。`canonical_payload_hash`=**规范化业务 payload 的摘要**（见下方 canonical 算法），recovery 分支内一切「是否同意图」的相等判断 SHALL **只比较此字段**。**「该父是否已恢复（resolved）」的唯一权威判定 SHALL 是因果键 `EXISTS(...)` 纯派生（见「recovery_blocked 的 resolved/unresolved 派生态」Requirement），SHALL NOT 以本表为 resolved 真相源**——本表是 recovery 分支建 child 的幂等/canonical 依据、是因果键的一致副本，不是第二套 resolved 判定来源。
  ...
  - **三分支（第十六轮 P1-E + 第十七轮 P1-D，比较字段统一为 `canonical_payload_hash`）**：**winner**（首次成功建 child）SHALL 与 child insert 同 `resume_blocked_execution()`（manual）或 reclaim 事务写入 `recovery_operations{operation_source, canonical_payload_hash, child}`;人工 winner 另在同事务写 `recovery_requests{outcome=winner, resolved_child_id=child}`;**loser·canonical 一致**（不同 token、`canonical_payload_hash` == 已存在 recovery_operations 行）回滚失败的 child-insert 后、独立事务写 `recovery_requests{outcome=idempotent_replay, resolved_child_id=已存在 child}`、返回 `idempotent_replay=true`;**loser·canonical 不同**独立事务写 `recovery_requests{outcome=rejected_already_resolved}`、返回 409 `already_resolved`（附赢家 child + 其 `canonical_payload_hash`）、SHALL NOT 静默映射不同意图到赢家 child。父子基数仍由 `UNIQUE(superseded_from)` + `recovery_operations.child_execution_id UNIQUE` 保证，`recovery_requests` 不承担父子基数。
- **🔴 两层预算严格分账（第十九轮 P1-1，SHALL NOT 混用）**：系统有两个语义不同的预算真相源，各扣各的、互不替代——
  - **`run_queue.execution_attempt_retry_remaining` + `execution_attempt_retry_version`（attempt 级重试预算，execution 维度，第十九轮 P1-1 + 第二十轮 P1-1 补完整可迁移 schema + 第二十一轮 P1-1 统一唯一公式）**：**仅**在**同一 execution 内重试**（瞬时失败 retryable、prestart failure retryable、protocol gate 未释放回队、**claimed lease reclaim 回队**）落 attempt 终态并回 `queued` 时扣减，**与 attempt 终态 + owner retire（epoch+1）+ `retry_scheduled` 事件同一 `finish_execution()`/回收事务**。**🔴 四类失败来源 SHALL 共用唯一 remaining 公式（第二十一轮 P1-1，消除 off-by-one 自相矛盾）——`remaining` 语义 = 「还可创建多少个后续 attempt」；失败时 CAS 读 old remaining：`old remaining>0 → 减 1 + 回 queued（该次 attempt 仍执行）`；`old remaining=0 → 当前 attempt 定局、不再回 queued`。** 即 `remaining=1` 扣成 0 的那次**仍回队执行**，下一次失败发现 `remaining=0` 才定局——瞬时/prestart/lease reclaim 类落终局 `failed`（计失败率、按 pending 建普通 successor），protocol gate 类**恰好一次**落 `recovery_blocked(protocol_incompatible)`。**四类来源 SHALL 用同一 table-driven 测试覆盖初值 0/1/2 × 崩溃重放 × 并发回收，断言产生的 attempt 次数一致（不因来源不同出现 N 或 N+1）**。此预算**不建 recovery child**、不动 `recovery_budgets`。schema 与生命周期 SHALL 固定（第二十轮 P1-1，用户拍板方案 A）：
    - **并发扣减方案 A（version CAS）**：`run_queue` 加 `execution_attempt_retry_version INTEGER NOT NULL DEFAULT 0`;每次回队扣减 SHALL 用 `UPDATE run_queue SET execution_attempt_retry_remaining = execution_attempt_retry_remaining - 1, execution_attempt_retry_version = execution_attempt_retry_version + 1 WHERE id=? AND execution_attempt_retry_remaining > 0 AND execution_attempt_retry_version = :expected`（remaining+version 双条件 CAS，崩溃重放/并发只扣一次，off-by-one 消歧）。
    - **初始值 + 配置来源**：`initial_retry_budget` 由 Paladin/配置提供（非硬编码），普通根 execution、普通 continuation successor、recovery child **创建时**均初始化 `execution_attempt_retry_remaining = initial_retry_budget`、`execution_attempt_retry_version = 0`（recovery child 是新 execution、其 attempt 重试预算独立于父，亦独立于 `recovery_budgets`）。
    - **存量回填 + 回滚**：expand 加两列（`remaining` 可空默认 NULL / `version` DEFAULT 0）→ 回填存量 queued/claimed/running execution 的 `remaining = initial_retry_budget`、`version = 0`（终态行不参与重试、回填 remaining=0 或保留 NULL 均可、不影响判定）→ activate 后新写入强制非空;回滚遵 compatibility floor（回滚到能读该列的兼容版）。
    - **`remaining = 1` 的最后一次**：CAS 命中 `remaining=1` 扣成 0 后仍完成本次回队（该次 attempt 仍执行），**下一次**失败发现 `remaining=0` 才落终局 `failed`/`recovery_blocked(protocol_incompatible)`，SHALL NOT 在 remaining 扣到 0 的当次直接拒绝回队（否则少一次重试机会、off-by-one）。
    - **lease reclaim 计入（用户拍板）**：`claimed lease 超时回收`（`claimed→abandoned→同 execution queued`）SHALL 同样在回收事务内扣 `execution_attempt_retry_remaining`，扣至 0 落终局 `failed`——**防 Worker 反复 claim 后崩溃导致无限 `claimed→abandoned→queued`**;lease reclaim 与瞬时/prestart/protocol gate 共用同一预算，不另设计数器。
...
- **🔴 独立 quarantine 表 + activate 双条件归零门（第十九轮 P1-4 + 第二十轮 P1-4：删「unknown 值 或 隔离表」二选一——`terminal_source_status` 值域恒只有三个业务值，`unknown` 不入值域，否则与值域约束/触发器「终态必为 queued/claimed/running」冲突）**：`terminal_source_status` 值域 SHALL **恒为 `{queued, claimed, running}`**（进终态时固化），SHALL NOT 出现 `unknown`。回填期推断不出 source status 的存量终态行 SHALL 落**独立隔离表** `terminal_source_backfill_quarantine`——
  ```text
  terminal_source_backfill_quarantine(
    execution_id PRIMARY KEY,
    reason,
    detected_at,
    resolved_at,   -- 人工核定后回填时间（NULL=待核）
    resolved_by
  )
  ```
  SHALL NOT 静默留 NULL 或猜一个值。**activate/readiness 前置门 SHALL 是双条件且 fail-closed（第二十轮 P1-4）**：
  ```text
  quarantine 未解决计数（resolved_at IS NULL）= 0
  AND 终态行 terminal_source_status IS NULL 计数 = 0
  → 才安装触发器、开启新读写 flag
  ```
  两条件任一非 0 即 fail-closed 拒绝 activate、readiness 不通过;先解决全部 quarantine（人工核定填回 `terminal_source_status` 三值之一 + 写 `resolved_at/by`、并从待核集移出）、再补齐所有终态行的 `terminal_source_status` 非空，才放行。
- **🔴 expand/activate 顺序 SHALL 先 dual-write 兼容再 activate（第十九轮 P1-4）**：① **expand** 加可空列（默认 NULL、旧版本忽略）→ ② **dual-write compatibility release**：先把**所有**写终态的 Writer（API/Worker/维护脚本）升级为「落终态必同写 `terminal_source_status`」的兼容版并全量部署，**SHALL NOT 只升级一部分 Writer**（否则 activate 后旧 Writer 仍可能写终态而漏写 source，产生新的 NULL/待核行）→ ③ **确定性回填 + 独立 `terminal_source_backfill_quarantine` 隔离表**（推断不了入隔离表、`terminal_source_status` 值域仍恒为三值，见下矩阵）→ ④ **双条件归零门**（quarantine 未解决=0 AND 终态 `terminal_source_status IS NULL`=0）→ ⑤ 安装 `BEFORE INSERT/UPDATE` 触发器（跨 status/blocked_reason/final/source 校验，终态 source 必为 queued/claimed/running）→ ⑥ **activate** 开新读写 flag（读侧启用 `final=NULL` 唯一例外判定、写侧强制非空 source）→ ⑦ observe（探针校验 CHECK 等价）→ ⑧ contract/native CHECK（若后续重建表）。回滚遵兼容读取版边界（activate 后回滚到能读该列的 compatibility release）。
- **🔴 历史回填确定性矩阵（第十九轮 P1-4，SHALL NOT 只写「推断 claimed/running」）**：对存量终态行按可判据确定性回填——① 有 `final_attempt_id` 且 attempt 有 `worker_generation/pid_create_time`（说明已启动过）→ source=running;② 有 `final_attempt_id` 但 attempt 为 `failure_stage=prestart`（claim 后未起 CLI）→ source=claimed;③ queued-NULL 隔离终态（`recovery_blocked AND null_conversation_migration AND final_attempt_id=NULL`）→ source=queued;④ `killed` 终态按其 attempt 是否曾 running 判 running/claimed（无 attempt→queued）;⑤ 历史缺 `failure_stage`/`worker_generation` 等关键字段无法判据 → **入 `terminal_source_backfill_quarantine` 隔离表待人工核定**（`terminal_source_status` 保持 NULL 不写 `unknown`，第二十轮 P1-4），SHALL NOT 猜测默认值。矩阵 SHALL 覆盖 done/failed/killed/superseded/recovery_blocked 各终态。
- 探针 `terminal_source_status_check_probe`（四条 CHECK 等价：终态非空/final=NULL 唯一例外/claimed·running 必有 final/非终态为 NULL）、`terminal_source_status_db_trigger_probe`（**第十九轮 P1-4**：绕过中心 helper 的直接 INSERT/UPDATE 写非法 status/source 组合被 BEFORE 触发器 `RAISE(ABORT)` 拒绝）、`terminal_source_status_backfill_matrix_probe`（**第十九轮 P1-4 + 第二十轮 P1-4**：running/prestart claimed/queued-NULL/killed 四类回填各判对应 source、缺字段类入 `terminal_source_backfill_quarantine` 隔离表且 `terminal_source_status` 保持 NULL 不写 unknown）、`terminal_source_status_activate_gate_probe`（**第十九轮 P1-4 + 第二十轮 P1-4**：双条件门——quarantine 未解决=0 AND 终态 `terminal_source_status IS NULL`=0，任一非 0 时 activate/readiness fail-closed、双双归零才放行）、`terminal_source_status_dualwrite_order_probe`（**第十九轮 P1-4**：dual-write 兼容版未全量部署即 activate 会产生新的 NULL/待核行、顺序门拒绝）、`terminal_source_status_value_domain_probe`（**第二十轮 P1-4**：`terminal_source_status` 只接受 `{queued,claimed,running}`，写 `unknown` 或其他值被值域约束/触发器拒绝）、`terminal_source_status_immutable_probe`（写入后 update 被守卫 + BEFORE UPDATE 触发器拒绝）。
...
#### Scenario: terminal_source_status 隔离表未清或有 NULL 终态时不得 activate（第十九轮 P1-4 + 第二十轮 P1-4）
- **WHEN** 历史回填后仍有存量终态行进了 `terminal_source_backfill_quarantine`（关键字段缺失无法判据、`terminal_source_status` 保持 NULL），或仍有终态行 `terminal_source_status IS NULL`，运维尝试 activate 新写入 flag 或系统做 readiness 检查
- **THEN** activate 双条件前置门检测到 `terminal_source_backfill_quarantine` 未解决计数 != 0 或 终态 `terminal_source_status IS NULL` 计数 != 0，fail-closed 拒绝 activate、readiness 不通过;只有隔离表全部核定归零 AND 终态 source 全部补齐非空后才放行 activate，SHALL NOT 在回填不完整时开启依赖 `terminal_source_status` 的 `final=NULL` 唯一例外判定
...
#### Scenario: dual-write 未全量部署即 activate 会产生新 NULL 终态（第十九轮 P1-4）
- **WHEN** 仅升级部分 Writer 为「落终态同写 terminal_source_status」的兼容版即 activate，剩余旧 Writer 仍写终态但漏写 source
- **THEN** 顺序门要求 dual-write compatibility release 全量部署（API/Worker/维护脚本全部）为 activate 前置;未满足即尝试 activate SHALL fail-closed 拒绝，避免 activate 后旧 Writer 持续产生新的 `terminal_source_status IS NULL` 非法终态

#### Scenario: terminal_source_status 值域拒绝 unknown（第二十轮 P1-4）
- **WHEN** 任何写入路径尝试给 `terminal_source_status` 写 `unknown` 或 `{queued,claimed,running}` 之外的值
- **THEN** 值域约束/BEFORE 触发器 `RAISE(ABORT)` 拒绝;推断不了的行只能进 `terminal_source_backfill_quarantine` 隔离表、`terminal_source_status` 保持 NULL，SHALL NOT 用 `unknown` 占位（否则与终态值域约束冲突）
