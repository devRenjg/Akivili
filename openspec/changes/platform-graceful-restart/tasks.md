...
- [ ] 1.5a2 **旧 `run_queue.task_run_id` 语义收口（Review 第四轮 P1-1）**：模型 A 后一 execution 多 attempt，旧列语义失配。三选一并全文引用同一选择——① 新代码统一走 `task_runs.run_queue_id` 反查、以获胜 attempt 为准，旧列仅 expand 期回填、contract 删;② 旧列重定义为「始终指向当前/最终获胜 attempt」并在每次 claim/终态原子维护;③ 新增 `winning_attempt_id`+`current_attempt_id` 淘汰旧列。**禁止「旧语义不变+多 attempt」模糊并存**（旧消费者会读到失败 attempt 而漏获胜 attempt）;探针 `legacy_task_run_pointer_probe`（attempt#1 失败、#2 成功，验旧列不指向失败 attempt 或已不被读取）
