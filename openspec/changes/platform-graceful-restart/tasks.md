...
**阶段 capability matrix（Review 第五轮 P1-3：消除前向依赖）**——阶段 1 的原子 claim 已依赖 `worker_state` 的 generation/owner/lease，故**最小 generation/lease 地基 SHALL 前移到阶段 1**（而非等阶段 5），否则研发无法判断 claim SQL 当时该校验什么、也无法独立验收阶段 1：

| 阶段 | 已存在字段/表 | 开启的 feature flag | 允许 Worker 数 | 必过验证门 |
|---|---|---|---|---|
| 1 | execution/attempt/event/`worker_state` **最小地基**（current_generation/owner_instance_id/lease_expires_at/protocol_version 字段就位，单 Worker 恒持有） | 新状态写入关闭 | 单 Worker | claim/finish 事务 |
| 2 | Worker 独立进程与 containment | API 内执行关闭 | 单 Worker | API 重启不中断 Worker |
...
| 6 | 蓝绿 API | 路由切换打开 | 双 API | SSE/兼容回滚探针 |

阶段 1 的 `worker_state` 最小地基只需保证「单 Worker 恒为当前 generation/owner、lease 持续续租」，claim CAS 即可稳定校验 generation/owner/lease;完整的多 generation 接管/交棒仍在阶段 5。
...
- [ ] 0.7 `--strict` 校验通过（已过）+ 第四轮 Review 复核认可（放行阶段 1）
...
- [ ] 1.5a2 **旧 `run_queue.task_run_id` 语义收口 = 方案③（Review 第五轮 P1-1 拍板）**：新增 `run_queue.current_attempt_id`（claim 时原子更新）+ `run_queue.winning_attempt_id`（终态时原子更新）+ `task_runs.run_queue_id`（反向归属）;expand 期由旧 `task_run_id` 回填两指针，新消费者切读 winning/current 后 contract 删旧列。**禁止「旧语义不变+多 attempt」模糊并存**（旧消费者会读到失败 attempt 而漏获胜 attempt）;探针 `legacy_task_run_pointer_probe`（attempt#1 失败、#2 成功，验消费者读 `winning_attempt_id` 得 #2、旧列不指向失败 attempt 或已不被读取）
- [ ] 1.5a3 **attempt 状态消费者矩阵（Review 第四轮 P1-2）**：补 `task_runs` attempt 层消费矩阵——各终态（succeeded/failed/killed/abandoned/prestart_failed/superseded）是否计失败率、Runtime/RunRow 展示、是否触发任务失败/自动流转;retryable running failure 用 `failed`+恢复计数;claimed 阶段被 kill→attempt/execution=`killed`;非重试 prestart failure→`prestart_failed`;非获胜 attempt（abandoned/superseded）SHALL NOT 计失败率或触发流转;attempt 允许转换表与 execution 转换表一并落地
...
- [ ] 1.6b **🔴 统一收尾事务 `finish_execution()`（Review 第五轮 P0-4，跨 PGR/ASR 单一提交）**：把「获胜 attempt 终态 + execution 终态 + session final 或 poisoned/fallback 清理 + committed 水位推进 + session owner retire + backlog/pending/recovery successor 创建及因果关联 + terminal/superseded/queued 事件写入（遵全局 id 顺序）」并入**一次提交**;任一 CAS/唯一约束失败整体回滚，调用方重读状态后退出或重试，不补偿式续提交;覆盖 pending/backlog/normal success/supersede/poisoned failure 五路径;ASR 的 session final/committed/owner retire SHALL 复用此同一事务，SHALL NOT 分别提交;探针 `combined_finish_transaction_probe`（每个 DB 写入点依次注入失败，验全提交或全回滚，覆盖五路径）
...
- [ ] 2.4 **进程 containment（Review P0-5/P0-D）**：Windows Job Object + `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE`;**POSIX 真 containment（Review 第五轮 P1-6：`start_new_session=True` ≠ 父死清理）**——拍板 `PR_SET_PDEATHSIG`（处理 fork/exec 竞态）/ 每 attempt 独立 cgroup / systemd scope `KillMode=control-group` 至少一种，且验证孙进程与工具子进程也清理（非只查 CLI 主 pid）;持久化 `pid + pid_create_time + worker_generation + worker_instance_id`（Review P1-2：attempt 行同时存 generation 与 instance id，注入 `jian` 环境供 fencing 比对）;探针 `posix_process_tree_cleanup_probe`（SIGKILL Worker + CLI 再拉孙进程，验证整棵子树退出）
- [ ] 2.4a **🔴 launch generation 最终启动围栏（Review 第五轮 P0-3）**：堵死「claim 成功→心跳暂停→新 Worker 接管 generation→旧 Worker 恢复起 CLI」时间窗。固定不可省略顺序——① 起进程前重校验 DB 当前 generation/instance/worker lease/attempt lease，任一不匹配立即 self-fence;② `CREATE_SUSPENDED` 起子进程 → `AssignProcessToJobObject`（POSIX 等价父死 containment），**尚未 Resume**;③ CAS 持久化 `pid+pid_create_time` 并转 attempt/execution 为 `running`，CAS **同时校验当前 `worker_state` generation/instance/lease**;④ **仅 CAS 成功才 `ResumeThread`**，失败立即销毁 suspended 进程树;⑤ 心跳/lease 续租失败后停 claim/停 launch/终止本 generation 进程并退出，不靠本地缓存;探针 `launch_epoch_barrier_probe`（suspended 创建前后切 generation，验旧 generation 用户代码零执行）、`heartbeat_self_fence_probe`（续租失败后恢复调度线程，验不 claim/launch/finalize/平台写）
