...
JianAgency 需要一个稳固的后端地基，才能在其上叠加 Agent 库、项目管理、对话与工作流。地基的第一要件，是**大模型 / CLI 多供应商配置**——这是整个执行引擎的输入。配置面较宽：不仅是 API（Deepseek/OpenAI/Anthropic），还包括本地 CLI 执行器（Claude Code、Codex）。用户需要在设置页配置多个供应商，并指定 Agent 默认用哪个。
...
- 前端新增设置页 `Settings.vue`：多供应商配置 Tab，预置 Claude Code / Codex / Deepseek / OpenAI / Anthropic / Ollama 选项
