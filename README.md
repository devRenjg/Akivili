...
- 企微推送：任务详情页「📮 发送到企微」一键把任务卡片（标题 + 交付正文 + 详情链接）推到企业微信群机器人，正文默认取最新交付、可编辑，webhook 配 config.json
...
### v0.16.22 — 2026-07-14
- ✨ **任务卡片一键推送企业微信群**（能力 `task-system`）
  - **背景**：希望把任务卡片的产出（如技术文档工程师摘要出的周会重点）自动化推送给企微团队群。
  - **接入方式（群机器人 webhook）**：在企微群添加「群机器人」拿到 webhook URL，配到 `config.json` 的 `wecom_webhook_url`（敏感凭证，gitignore 已排除；也可用环境变量 `AKIVILI_WECOM_WEBHOOK`）。另配 `frontend_base_url`（如 `http://10.23.28.27:8100`）用于拼「详情请点击」的任务卡片链接。无需企微管理员建应用，最轻量。
  - **触发（手动按钮）**：任务详情页右上角「📮 发送到企微」→ 弹框预填「本任务最新交付正文」（可编辑）+ 可加一段补充说明 → 一键发送。推送内容 = `# 任务标题` + 补充说明 + 正文摘要 + `> 详情请点击：任务卡片链接`。
  - **后端**：新增 `backend/wecom.py`（markdown 组装 + UTF-8 字节安全截断到企微 4096 上限 + httpx 异步发送 + 结果归一化）+ `POST /tasks/{id}/push-wecom`（管理员鉴权；webhook 未配返回明确 400 提示；企微 errcode 非 0 如实回传 502，绝不静默失败；错误不带 webhook 明文）。
  - ⚠️ **上线需重启后端**加载新配置字段与路由；使用前需在 `config.json` 配好 `wecom_webhook_url` 与 `frontend_base_url`。
  - 验证：新增 `TestReport/run_wecom_push_probe.py` **12/12**（markdown 结构、字节安全截断不切坏汉字、空链接/副标题降级、send 结果归一化含网络异常，monkeypatch httpx 不发真实请求）；前端 `npm run build` 通过；回归 QA 31/31。
