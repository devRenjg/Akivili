...
- 循环闸：单任务末尾**连续空转** mention 链达 `MAX_MENTION_CHAIN`（默认 8）即停止入队。**「产出即重置」口径**——只累计「连续多棒 @ 却谁都没产出」的真死循环空转；链中任一棒产出了真实交付（该 run 落过带 `run_id` 的 jian comment/subtask 发言）即重置链。故多轮复验/返工这种长链协作（每棒都有交付）不被误掐，纯 @ 空转仍如期熔断（修 task149 误伤事故）。人工介入/负责人统筹/人工直接 @ 也打断链。
...
- 🔧 **引擎抽象 + SQLite/PostgreSQL 双跑（能力 `foundation-data-layer`，OpenSpec change `2026-07-24-foundation-db-alignment`，回滚锚点 D）**
  ...
  - **S4.5 数据迁移 + 严格一致性**：`backend/migrate_sqlite_to_pg.py`（只读源库、依赖序插入、保留原始 id、迁移后重置 PG 序列）+ `TestReport/run_pg_sqlite_consistency_probe.py`（逐表行数 + **逐行逐列全量比对**）。迁移中处理了 3 个 SQLite 宽松/PG 严格暴露的真实数据问题：**NUL 字节剔除**（2 字段，PG text 不接受 `\0`）、**8 行悬空外键跳过**（`run_queue` 7 + `activities` 1 指向已删 task，SQLite 外键默认关闭积累的孤儿）、**id 不连续保留 + 序列重置**。
...
- ✨ **运行时页新增「实时 Agent 总览」+ 下线限流/429 展示**（能力 `agent-collaboration`）
  ...
  - **下线限流展示（保留大数字）**：运行时页移除 429/命中率面板与相关前端逻辑（`rl*`）。后端 `/api/runs/rate-limit-metrics` 接口**保留不动**（以后需要再接回），只是前端不再调用展示。
  - **新增实时总览**：后端新增 `GET /api/runs/agents-overview?days=N`——① `stats` 窗口口径大数字（跑过多少 run、多少失败、涉及多少去重 Agent、**所有 Agent 累计运行总时长** = 已结束 run 的 `ended_at-started_at` 求和）；② `running` 正在执行的 Agent（以 `task_runs.status='running'` 为准，覆盖并发池 + 直接 @ 两条路径），带项目/任务/子任务标记/开始时间；③ `idle` 启用成员里此刻无 running run 的（显示 idle）。身份粒度为 `(project_id, slug)`——同一花名册成员在不同项目算不同在岗实例；running 与 idle 严格分区互斥；展示名统一「昵称（角色名）」。**累计口径可按时间窗口筛选**（最近一周=7 / 默认最近一个月=30 / 最近半年=180 / 最近一年=365 / 用户自填天数 1~365，后端默认 30、clamp 1~365），实时的 running/idle 不受窗口影响。前端 `Runtime.vue` 顶部改为撑满居中的**大面板**（40px 大数字居中一排 + 窗口筛选器），运行中/空闲改为**纵向行列表**，每 10s 自动刷新，点行可跳到对应任务/项目的历史链路。**与「按任务筛选看历史链路」并存，事后复盘视角不受影响。**
  - ⚠️ **上线需重启后端**加载新接口；前端需 `npm run build`。重启前务必杀净所有 8100 监听进程（reload 模式下曾出现两套后端并存、请求随机命中旧代码致新接口时而 404，本次已清成单实例）。
  - 验证：新增 `TestReport/run_agents_overview_probe.py` **22/22**（stats 窗口口径且不含限流字段、总时长只算窗口内已结束 run、`days` 窗口过滤与越界 clamp、running 带项目/任务/展示名/子任务标记、idle 排除运行中与禁用成员、跨项目同 slug 算两个在岗实例、running∩idle 互斥）；全量回归 23 个探针全绿（含保留的 `run_rate_limit_probe` 8/8）。

### v0.16.22 — 2026-07-14
- ✨ **展示名统一为「昵称（角色名）」+ 强化 @ 协议提示（task140 停链治本）**（能力 `agent-collaboration`）
  - **背景**：task140 后期负责人交办卡芙卡后「没了后续」，排查确认 run#253 发言写的是「卡芙卡，你来做…」——**只喊昵称、没带 `@` 符号**，`parse_and_enqueue_mentions` 只认带 `@` 的花名册名字，于是没人被唤醒、链自然停。匹配逻辑本身没 bug（`@卡芙卡` 昵称/`@后端架构师` 角色名都能触发），是 Agent 发言不规范。同时运行时/链路页面此前显示裸 slug（如 `engineering-backend-architect`），可读性差。
  - **治本口径（保 @ 触发 + 强化协议提示）**：不改「必须带 `@`」的硬规则（隐式匹配裸名字会误伤正文里提到的人名），改为在协议与花名册里把「`@` 符号强制、只喊名字不带 `@` 不唤醒任何人」讲死——`TEAM_LEADER_PROTOCOL` 规则 2 与 `build_roster` 委派行都新增显式警示，`@后端架构师`/`@卡芙卡` 皆可但**一定要有 `@`**。
  - **展示名统一**：`collab.resolve_agent_displays(slugs)` 批量把 slug 解析为「昵称（角色名）」（无昵称则只显角色名，查不到回退 slug）；`routes/runs.py` 的链路（`get_lineage`）与转写元信息接口补 `agent_display` 字段，前端运行时页 `Runtime.vue` 改显 `agent_display`。页面、用户 @ 成员、运行时三处统一，不再露裸 slug。
  - **回滚**：撤销上一版「任务卡片一键推送企业微信群」功能与按钮（用户另有方案），revert 提交 `0cc2de5`。
  - ⚠️ **上线需重启后端**加载新协议文案与展示名解析；前端需 `npm run build`。
  - 验证：`run_mention_prompt_probe.py` **13/13**（新增：`@昵称` 触发对应成员、只写名字不带 `@` 不触发任何人——钉死 #451 停链真因）、`run_lineage_probe.py` **13/13**（新增链路项 `agent_display` 为展示名非裸 slug）；顺带修 `run_timeout_and_qa_probe.py` 场景 D 的自测确定性缺陷（假时钟复刻硬墙钟续期语义）14/14、补齐 `run_scheduling_probe.py` 的 `_FakeSettings` 闸配置字段 10/10；全量回归 22 个探针全绿。

### v0.16.21 — 2026-07-14
- 🐛 **修复循环闸误伤多轮真实协作——task149 收尾被误判死循环掐断**（能力 `agent-collaboration`）
  - **背景（事故复盘）**：task149（卡芙卡返工·数据完整性）后期，后端架构师→产品裁决→高级开发者复验来回多棒真实协作，每棒都有实打实交付（改代码、跑复算、出裁决），但从 19:17 起反复打出「检测到连续 8 次 @ 链式自动协同（疑似循环），自动停止以防烧钱」，负责人验收/后续派活全被掐断，任务像「没了后续」。
  - **根因**：循环闸（`_mention_chain_len`）旧口径只数「末尾连续 mention 链式自动 run 的**条数**」，达 `MAX_MENTION_CHAIN`（8）即熔断。它区分不了「Agent 互相 @ 空转的死循环」和「多轮有产出的实质协作」——task149 从一次 assign 后连续 8 棒 mention 协作，条数正好撞 8，被误判成死循环。
  - **修复（产出即重置）**：`_mention_chain_len` 改为只累计末尾连续**空转**（无产出）的 mention 链；从最新往回数，一旦遇到某棒产出了真实交付（该 run 落过带 `run_id` 的 assistant 消息 = jian comment/subtask，新增 `_run_queue_item_produced` 判定）即重置链、停止计数。故有产出的长链协作不被误掐，纯 @ 空转（连续多棒无产出）仍如期累积熔断，防死循环保护不削弱。用精确的「run_id 绑定的消息」作产出信号，不用「该 slug 在本任务发过言」宽口径（否则一个 Agent 早先发过言、其后所有空转都被误判有产出，闸永不触发）。
  - 实测：task149 新口径链长从 8 → 1（最新一棒无产出、往回一棒有产出即重置），不再被拦。
  - ⚠️ **上线需重启后端**加载新判定逻辑。task149 的收尾可在网页人工 @ 一次继续（人工介入本就打断链）。
  - 验证：新增 `TestReport/run_mention_chain_reset_probe.py` **6/6**（有产出长链重置为 0 不误掐、纯空转链仍累积熔断、混合链只数末尾空转、run 没起来算空转）；回归 QA 31/31、task_gates 10/10、scheduling 10/10、scheduling_events 6/6、subtask 6/6、orphan_leak 11/11。
...
- 📈 **端到端链路可观测性·阶段三 P3-3：运行时可视化时间线**（能力 `agent-collaboration`/`agent-execution`）
  ...
  - **可视化时间线**：每个 run 一个时间线节点，节点色区分状态（绿=完成/红=失败或终止/橙=执行中或排队）。run 头行展示负责人标记、成员、触发方式、状态、失败原因、重试次数、单 run 耗时、入队时间；展开后见 task_id/run_queue/task_run 关联（点 task_run 直接调 `RunTranscriptDialog` 看执行记录）、`source_run_id/message_id` 因果来源、执行窗口、以及 `run_events` 调度流水逐条（入队→领取→重试→完成/失败，带各自时间戳）。
  - **汇总条**：关联任务数、执行 run 数、链路总耗时、失败 run 数一眼可见。
  - **任务详情页深链**：任务详情右侧「执行日志」块新增「链路 ↗」入口，子任务自动归到父任务链根，一键跳到该任务的时间线；URL 带 `?project=&task=` 便于分享/回跳。
  - **取舍**：视图为 lineage 接口的纯渲染层，不引入新后端；P3-3 落地后可观测性缺口评估的三阶段全部闭环。
  - 验证：`run_lineage_probe.py` 扩展至 **12/12**（新增 3 条前端时间线字段契约断言，锁定汇总/链路项/流水项视图所需字段，防后端改动静默破坏 `Runtime.vue`）；前端 `npm run build` 通过。
