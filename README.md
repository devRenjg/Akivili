...
- ✨ **运行时页新增「实时 Agent 总览」+ 下线限流/429 展示**（能力 `agent-collaboration`）
  ...
  - **新增实时总览**：后端新增 `GET /api/runs/agents-overview`——① `stats` 累计大数字（跑过多少 run、多少失败、涉及多少去重 Agent）；② `running` 正在执行的 Agent（以 `task_runs.status='running'` 为准，覆盖并发池 + 直接 @ 两条路径），带项目/任务/子任务标记/开始时间；③ `idle` 启用成员里此刻无 running run 的（显示 idle）。身份粒度为 `(project_id, slug)`——同一花名册成员在不同项目算不同在岗实例；running 与 idle 严格分区互斥；展示名统一「昵称（角色名）」。前端 `Runtime.vue` 顶部换成总览区（大数字 + 运行中卡片 + 空闲 chip），每 10s 自动刷新，点卡片可跳到对应任务/项目的历史链路。**与「按任务筛选看历史链路」并存，事后复盘视角不受影响。**
  - ⚠️ **上线需重启后端**加载新接口；前端需 `npm run build`。重启前务必杀净所有 8100 监听进程（reload 模式下曾出现两套后端并存、请求随机命中旧代码致新接口时而 404，本次已清成单实例）。
  - 验证：新增 `TestReport/run_agents_overview_probe.py` **17/17**（stats 累计口径且不含限流字段、running 带项目/任务/展示名/子任务标记、idle 排除运行中与禁用成员、跨项目同 slug 算两个在岗实例、running∩idle 互斥）；全量回归 23 个探针全绿（含保留的 `run_rate_limit_probe` 8/8）。
