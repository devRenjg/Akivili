...
"""
from sqlalchemy import and_, exists, func, select, update as sa_update

from models import RunQueue, Task, get_session_factory, now_expr
...
async def blocking_subtasks(task_id: int) -> list[dict]:
    ...
    async with get_session_factory()() as session:
        rows = (await session.execute(
            select(Task.id, Task.title, Task.assignee_slug, Task.status)
            .where(Task.parent_task_id == task_id,
                   exists().where(and_(RunQueue.task_id == Task.id,
                                       RunQueue.status.in_(("queued", "running")))))
        )).all()
        return [{"id": r.id, "title": r.title, "assignee_slug": r.assignee_slug,
...
async def task_progress(task_id: int) -> dict:
    ...
    async with get_session_factory()() as session:
        subs = (await session.execute(
            select(Task.id, Task.status).where(Task.parent_task_id == task_id))).all()
        ...
        runs = (await session.execute(
            select(RunQueue.task_id, RunQueue.agent_slug, RunQueue.status)
            .where(RunQueue.task_id.in_(all_ids),
                   RunQueue.status.in_(("queued", "running")))
            .order_by(RunQueue.id))).all()
        # 负责人是否已做过收尾汇总：父任务上有一条以负责人身份完成的 collaborate run
        parent = (await session.execute(
            select(Task.status).where(Task.id == task_id))).first()
        parent_status = parent.status if parent else ""
        summ = (await session.execute(
            select(RunQueue.id).where(RunQueue.task_id == task_id, RunQueue.is_leader == 1,
                                      RunQueue.trigger == "collaborate", RunQueue.status == "done")
            .limit(1))).first()
...
async def _has_pending_run(task_ids: list[int], exclude_run_id: int | None = None) -> bool:
    ...
    async with get_session_factory()() as session:
        stmt = (select(func.count())
                ...
                .where(RunQueue.task_id.in_(task_ids),
                       RunQueue.status.in_(("queued", "running"))))
        if exclude_run_id is not None:
            stmt = stmt.where(RunQueue.id != exclude_run_id)
...
async def _set_reviewing(task_id: int, note: str) -> bool:
    ...
    async with get_session_factory()() as session:
        row = (await session.execute(select(Task.status).where(Task.id == task_id))).first()
        if not row or row.status not in ("in_progress", "backlog", "planning"):
            return False
        old = row.status
        await session.execute(sa_update(Task).where(Task.id == task_id).values(
            status="reviewing", updated_at=now_expr()))
...
async def _set_done(task_id: int, note: str) -> bool:
    ...
    async with get_session_factory()() as session:
        ...
        old = row.status
        await session.execute(sa_update(Task).where(Task.id == task_id).values(
            status="done", updated_at=now_expr()))
        await session.commit()
    from activity import log_activity
    await log_activity(task_id, "status_changed", "system", "",
...
async def _qa_member_hint(parent_id: int) -> str:
    ...
    from models import ProjectAgent
    async with get_session_factory()() as session:
        prow = (await session.execute(
            select(Task.project_id).where(Task.id == parent_id))).first()
        if not prow:
            return ""
        rows = (await session.execute(
            select(ProjectAgent.slug, ProjectAgent.name)
            .where(ProjectAgent.project_id == prow.project_id))).all()
...
async def _advance_and_summarize_parent(parent_id: int) -> None:
    ...
    async with get_session_factory()() as session:
        parent = (await session.execute(
            select(Task.title, Task.assignee_slug).where(Task.id == parent_id))).first()
        subs = (await session.execute(
            select(Task.id, Task.title, Task.assignee_slug)
            .where(Task.parent_task_id == parent_id))).all()
    leader_slug = parent.assignee_slug if parent else ""
    if not leader_slug:
        return  # 无负责人（历史任务）：仅置 reviewing，不唤醒
    parent_title = parent.title if parent else ""
    sub_list = [{"id": s.id, "title": s.title, "assignee_slug": s.assignee_slug} for s in subs]
...
async def on_execution_complete(task_id: int, exclude_run_id: int | None = None) -> None:
    ...
    async with get_session_factory()() as session:
        t = (await session.execute(
            select(Task.id, Task.parent_task_id, Task.status).where(Task.id == task_id))).first()
        if not t:
            return
    ...
    if parent_id:
        ...
        async with get_session_factory()() as session:
            subs = (await session.execute(
                select(Task.id, Task.status).where(Task.parent_task_id == parent_id))).all()
        sub_ids = [r.id for r in subs]
        ...
        async with get_session_factory()() as session:
            sub_ids = [r.id for r in (await session.execute(
                select(Task.id).where(Task.parent_task_id == task_id))).all()]
...
async def maybe_advance_parent(sub_task_id: int) -> None:
    ...
    async with get_session_factory()() as session:
        row = (await session.execute(
            select(Task.parent_task_id).where(Task.id == sub_task_id))).first()
        ...
        parent = (await session.execute(
            select(Task.title, Task.project_id, Task.assignee_slug, Task.status)
            .where(Task.id == parent_id))).first()
        parent_status = parent.status if parent else ""
        parent_title = parent.title if parent else ""
        project_id = parent.project_id if parent else 0
        leader_slug = parent.assignee_slug if parent else ""
        sub_list = [{"id": s.id, "title": s.title, "assignee_slug": s.assignee_slug,
    ...
    # 1) 父任务 → reviewing
    async with get_session_factory()() as session:
        await session.execute(sa_update(Task).where(Task.id == parent_id).values(
            status="reviewing", updated_at=now_expr()))
        await session.commit()
    from activity import log_activity
